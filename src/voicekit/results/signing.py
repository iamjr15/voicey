...
def encode_secret(key: bytes) -> str:
    ...
    if not key:
        raise VoicekitError("VK-RES-001", detail="The decoded key is empty.")
...
class WebhookSigner:
    ...
    def sign(
        ...
        *,
        ...
        if not event_id:
            raise VoicekitError("VK-RES-002", detail="webhook-id is empty.")
    ...
    def verify(
        ...
        *,
        ...
        try:
            ...
            raise VoicekitError("VK-RES-002") from exc
        if not event_id or not signature_text:
            raise VoicekitError("VK-RES-002")

        current_time = int(time.time()) if now is None else now
        if abs(current_time - timestamp) > tolerance_seconds:
            raise VoicekitError("VK-RES-003")
...
def _decode_secret(secret: str) -> bytes:
    ...
    try:
        ...
        raise VoicekitError("VK-RES-001", detail="The key is not valid base64.") from exc
    if not key:
        raise VoicekitError("VK-RES-001", detail="The decoded key is empty.")
    return key
