...
class UpgradeManager:
    """Upgrade only voicekit's lock entry, sync, and inspect with the new process."""
    ...
    def upgrade(
        ...
        *,
        ...
        if self.lockfile.is_symlink():
            raise VoicekitError("VK-SEC-002", detail=str(self.lockfile))
        ...
        try:
            ...
            locked_version = self._locked_voicekit_version()
            if not prerelease and _is_prerelease(locked_version):
                raise VoicekitError(
                    "VK-UPG-002",
                    detail=(
                        "stable resolution selected a voicekit prerelease; "
        ...
        return UpgradeReport(
            ...
            recipe_drift=drift,
            next_step=str(drift.get("next_step") or "voicekit doctor"),
        )
    ...
    def _validate_project(self) -> bytes:
        ...
        if not isinstance(dependencies, list) or not any(
            isinstance(value, str) and _dependency_name(value) == "voicekit"
            for value in cast("list[object]", dependencies)
        ):
            raise VoicekitError(
                "VK-UPG-001",
                detail="project.dependencies must contain a direct voicekit requirement.",
            )
        return payload
    ...
    def _validate_uv(self) -> None:
        ...
        if match is None:
            raise VoicekitError("VK-UPG-001", detail="uv did not report a semantic version.")
        version = tuple(int(part) for part in match.groups())
        if not _UV_MINIMUM <= version < _UV_MAXIMUM:
            raise VoicekitError(
                "VK-UPG-001",
                detail="voicekit upgrade requires uv >=0.11,<1.",
            )
    ...
    def _locked_voicekit_version(self) -> str:
        ...
        try:
            ...
            raise VoicekitError("VK-UPG-002", detail="uv.lock is invalid.") from exc
        packages = cast("dict[str, object]", parsed).get("package")
        if not isinstance(packages, list):
            raise VoicekitError("VK-UPG-002", detail="uv.lock has no package table.")
        ...
        for item in cast("list[object]", packages):
            ...
            if name == "voicekit" and isinstance(version, str):
                versions.add(version)
        if len(versions) != 1:
            raise VoicekitError(
                "VK-UPG-002",
                detail="uv.lock does not identify one voicekit version.",
            )
        return versions.pop()
...
def _is_prerelease(value: str) -> bool:
    try:
        ...
        raise VoicekitError(
            "VK-UPG-002",
            detail="uv.lock contains an invalid voicekit version.",
        ) from exc
...
def _atomic_write(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        raise VoicekitError("VK-UPG-002", detail=f"could not restore {path}.") from exc
