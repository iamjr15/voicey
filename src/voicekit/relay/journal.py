...
from voicekit.security.files import ensure_private_file
...
class SQLiteRelayJournal:
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            raise VoicekitError("VK-REL-006", detail="could not close relay journal.") from exc
    ...
    async def ready(self) -> bool:
        ...
        try:
            ...
            raise VoicekitError("VK-REL-006", detail="relay journal readiness failed.") from exc
        ...
        *,
        ...
        async with self._lock:
            try:
                ...
                await database.rollback()
                raise VoicekitError("VK-REL-006", detail=detail) from exc
        if changed != 1:
            raise VoicekitError("VK-REL-005", detail=detail)

    def _connection(self) -> aiosqlite.Connection:
        if self._database is None:
            raise VoicekitError("VK-REL-006", detail="relay journal is not open.")
        return self._database


def _iso(value: datetime) -> str:
    if value.tzinfo is None or value.utcoffset() is None:
        raise VoicekitError("VK-REL-006", detail="relay journal timestamp is naive.")
