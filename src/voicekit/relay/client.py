...
from voicekit.relay.auth import RelayCredential, RelayRequestSigner
...
class RelayClient:
    ...
    async def open(self) -> Self:
        ...
        try:
            ...
            raise VoicekitError("VK-REL-002", detail="relay readiness payload is invalid.") from exc
    ...
    async def renew_lease(
        ...
        *,
        ...
        if not isinstance(value, dict):
            raise VoicekitError("VK-REL-006", detail="relay renewal acknowledgement is invalid.")
        try:
            renewed = CallLease.model_validate(value)
        except ValidationError as exc:
            raise VoicekitError("VK-REL-006", detail="relay renewal lease is invalid.") from exc
    ...
    async def get_call(self, call_id: str) -> CallRecord:
        ...
        try:
            ...
            raise VoicekitError("VK-REL-006", detail="relay call response is invalid.") from exc
        ...
        *,
        ...
        async with state.lock:
            ...
            if response.sequence != state.next_sequence:
                raise VoicekitError("VK-REL-005", detail="relay acknowledged a different sequence.")
        ...
        for attempt in range(self._max_attempts):
            try:
                ...
                if response.status_code < 400:
                    ...
                    raise VoicekitError("VK-REL-006", detail="relay response is not an object.")
...
def _event_result(response: RelayUpdateResponse) -> PersistedEvent:
    ...
    if not isinstance(value, dict):
        raise VoicekitError("VK-REL-006", detail="relay event acknowledgement is invalid.")
    try:
        return PersistedEvent.model_validate(value)
    except ValidationError as exc:
        raise VoicekitError("VK-REL-006", detail="relay event payload is invalid.") from exc


def _response_error(response: httpx.Response) -> VoicekitError:
    try:
        ...
        if isinstance(payload, dict):
            ...
            if isinstance(error, dict):
                ...
                if isinstance(code, str) and isinstance(detail, str):
                    return VoicekitError(code, detail=detail)
    except (json.JSONDecodeError, AssertionError):
        pass
    return VoicekitError(
        "VK-REL-002" if response.status_code >= 500 else "VK-REL-006",
