"""SemVer transition and deprecation policy for Voicekit's public surface."""
...
class Deprecation:
    ...
    def message(self) -> str:
        return (
            f"{self.name} is deprecated since Voicekit {self.since} and will be removed "
            f"in {self.remove_in}. Use {self.replacement}. Migration: {self.migration_url}"
        )


class VoicekitDeprecationWarning(FutureWarning):
...
def warn_deprecated(deprecation: Deprecation, *, stacklevel: int = 2) -> None:
    """Emit Voicekit's standard actionable deprecation warning."""
    warnings.warn(
        deprecation.message,
