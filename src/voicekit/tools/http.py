...
from voicekit.tools.core import ToolMetadata, set_tool_metadata, validate_tool_name
    ...
    def __init__(
        ...
        *,
        ...
        if normalized_method not in {"GET", "POST", "PUT", "PATCH", "DELETE"}:
            raise VoicekitError("VK-TOL-004", detail=f"unsupported method {method!r}.")
        if timeout_s <= 0:
            raise VoicekitError("VK-TOL-004", detail="timeout_s must be positive.")
        parsed_url = urlsplit(url)
        if parsed_url.scheme not in {"http", "https"} or not parsed_url.hostname:
            raise VoicekitError("VK-TOL-004", detail=f"invalid HTTP tool URL {url!r}.")
        ...
        try:
            ...
            raise VoicekitError("VK-TOL-004", detail="invalid URL parameter template.") from exc
...
def _expand_environment(template: str) -> str:
    ...
    for name in names:
        ...
        if not value:
            raise VoicekitError("VK-TOL-004", detail=f"missing environment variable {name}.")
