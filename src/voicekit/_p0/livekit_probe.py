...
async def run_livekit_probe() -> RuntimeProbe:
    ...
    @server.rtc_session(agent_name="voicekit-p0")
...
def _issue_browser_token() -> str:
    return (
        api.AccessToken(
            "voicekit-p0-key",
            "voicekit-p0-development-secret-at-least-32-bytes",
        )
        .with_identity("p0-browser")
        .with_grants(api.VideoGrants(room_join=True, room="voicekit-p0-room"))
        .with_room_config(
            api.RoomConfiguration(agents=[api.RoomAgentDispatch(agent_name="voicekit-p0")])
        )
        .to_jwt()
    )
