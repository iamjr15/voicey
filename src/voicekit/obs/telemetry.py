...
from voicekit.config.models import Agent, Observability
from voicekit.errors import ERROR_CATALOG, VoicekitError
from voicekit.obs.latency import LatencySample
from voicekit.obs.records import NewCall, ToolCallObservation, TranscriptTurn
...
_FAILURE_CODE: dict[EndedReason, str] = {
    "stt_unavailable": "VK-RUN-002",
    "llm_unavailable": "VK-RUN-002",
    "tts_unavailable": "VK-RUN-002",
    "carrier_error": "VK-TEL-011",
    "provider_error": "VK-RUN-002",
    "worker_crash": "VK-RUN-006",
    "setup_error": "VK-RUN-003",
    "recovery_unknown": "VK-RUN-006",
    "unknown": "VK-RUN-006",
}
_LATENCY_BUCKETS_MS = (
    25,
...
class Telemetry:
    ...
    def __init__(
        ...
        *,
        ...
        self._calls = Counter(
            "voicekit_calls",
            "Durably admitted calls; use rate(voicekit_calls_total[5m]) for call rate.",
            labels,
            registry=self.registry,
        )
        self._active = Gauge(
            "voicekit_active_calls",
            "Calls currently owned by this voicekit process.",
            labels,
            registry=self.registry,
        )
        self._errors = Counter(
            "voicekit_errors",
            "Errors grouped by stable voicekit catalog code.",
            (*labels, "code"),
            registry=self.registry,
        )
        self._dlq = Gauge(
            "voicekit_results_dlq_depth",
            "Current durable result-delivery dead-letter depth.",
            labels,
            registry=self.registry,
        )
        self._latency = Histogram(
            "voicekit_turn_latency_ms",
    ...
    def begin_call(self, call: NewCall, *, count: bool = True) -> None:
        ...
        with self._lock:
            ...
            self._call_spans[call.call_id] = tracer.start_span(
                ...
                attributes={
                    "voicekit.call.id": call.call_id,
                    "voicekit.agent.name": call.agent_name,
                    "voicekit.runtime": call.runtime,
                    "voicekit.channel": call.channel,
                    "voicekit.direction": call.direction,
                    **(
                        {}
                        if call.provider is None
                        else {"voicekit.telephony.provider": call.provider}
                    ),
                },
            )
    ...
    def finish_call(self, call_id: str, request: TerminalRequest) -> None:
        ...
        if span is not None:
            span.set_attribute("voicekit.call.event_type", request.event_type)
            span.set_attribute("voicekit.call.ended_reason", request.ended_reason)
            if code is not None:
    ...
    def observe_turn(self, call_id: str, turn: TranscriptTurn) -> None:
        self._child_span(
            ...
            "voicekit.turn",
            {
                "voicekit.turn.id": turn.turn_id,
                "voicekit.turn.role": turn.role,
                "voicekit.turn.offset_ms": turn.t_ms,
            },
        )
    ...
    def observe_tool(self, call_id: str, observation: ToolCallObservation) -> None:
        self._child_span(
            ...
            "voicekit.tool",
            {
                "voicekit.tool.name": observation.tool_name,
                "voicekit.tool.status": observation.status,
                "voicekit.tool.duration_ms": observation.duration_ms,
            },
    ...
    def set_dlq_depth(self, depth: int) -> None:
        if depth < 0:
            raise VoicekitError("VK-OBS-006", detail="DLQ depth cannot be negative.")
    ...
    def _ensure_tracer(self) -> trace.Tracer | None:
        ...
        try:
            ...
            provider = TracerProvider(
                resource=Resource.create(
                    {
                        "service.name": "voicekit",
                        "service.instance.id": f"{self.agent_name}-{pid}",
                        "voicekit.agent.name": self.agent_name,
                        "voicekit.runtime": self.runtime,
                    }
                ),
        ...
        self._tracer = provider.get_tracer("voicekit", "1")
        return self._tracer
...
class TelemetryServer:
    ...
    async def start(self) -> None:
        ...
        self._task = asyncio.create_task(self._server.serve(), name="voicekit-metrics")
        try:
            await asyncio.wait_for(self._wait_started(), timeout=10)
        except Exception as exc:
            await self.stop()
            if isinstance(exc, VoicekitError):
                raise
            raise VoicekitError(
                "VK-OBS-006",
