...
class LiveKitCloudSessionSmoke:
    ...
    async def run(
        ...
        *,
        ...
        room_name = f"vk-smoke-{uuid.uuid4().hex}"
        reservation_owner = f"livekit_reservation_{call_id}"
        phone_smoke = to_number is not None
        trunk_id = (
            _required(environment, "VOICEKIT_LIVEKIT_OUTBOUND_TRUNK_ID") if phone_smoke else None
        )
            ...
            if phone_smoke:
                await client.sip.create_sip_participant(
                    api.CreateSIPParticipantRequest(
                        ...
                        room_name=room_name,
                        participant_identity=f"voicekit-smoke-{uuid.uuid4().hex}",
                        participant_name="voicekit cloud smoke",
...
def _required(environment: Mapping[str, str], name: str) -> str:
    ...
    if not value:
        raise VoicekitError("VK-DEP-008", detail=f"LiveKit Cloud smoke requires {name}.")
    return value
