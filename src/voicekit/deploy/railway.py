...
class RailwayArtifacts:
    """Secret-free Railway build context owned by voicekit."""
...
class RailwayArtifactGenerator:
    ...
    def __init__(self, project_root: Path) -> None:
        ...
        self.directory = self.project_root / ".voicekit" / "deploy" / "railway"
...
class RailwayDeploymentManager:
    ...
    def __init__(
        ...
        *,
        ...
        self.store = RailwayResourceStore(
            self.project_root / ".voicekit" / "deploy" / "railway-resources.json"
        )
    ...
    def rollback_created(self, plan: RailwayPlan) -> RailwayResourceState:
        """Delete only identities explicitly marked created by voicekit."""
        state = self.store.load()
        if state is None:
            raise VoicekitError("VK-DEP-007", detail="no Railway resource ledger exists.")
        state.validate_plan(plan)
        self._authenticate()
        self._link(state)
    ...
    def _ensure_project(
        ...
        if len({_item_text(item, "id") for item in matches}) > 1:
            raise VoicekitError("VK-DEP-007", detail="Railway project identity is ambiguous.")
        if state.project_id is not None:
            ...
            if _item_text(project, "name") != plan.project_name:
                raise VoicekitError("VK-DEP-007", detail="Railway project name drifted.")
    ...
    def _ensure_service(
        ...
        if len(matches) > 1:
            raise VoicekitError("VK-DEP-007", detail="Railway service identity is ambiguous.")
        if matches:
            ...
            if state.service_id not in {None, service_id}:
                raise VoicekitError("VK-DEP-007", detail="Railway service id drifted.")
            checkpoint = state.checkpoint(service_id=service_id)
            self.store.save(checkpoint)
            return checkpoint
        if state.service_id is not None:
            raise VoicekitError("VK-DEP-007", detail="ledgered Railway service is missing.")
    ...
    def _ensure_postgres(
        ...
        if len(matches) > 1:
            raise VoicekitError("VK-DEP-007", detail="Railway Postgres identity is ambiguous.")
        if matches:
            postgres_id = _item_text(matches[0], "id")
            postgres_name = _item_text(matches[0], "name")
            if state.postgres_id not in {None, postgres_id}:
                raise VoicekitError("VK-DEP-007", detail="Railway Postgres id drifted.")
            checkpoint = state.checkpoint(
        ...
        if state.postgres_id is not None:
            raise VoicekitError("VK-DEP-007", detail="ledgered Railway Postgres is missing.")
    ...
    def _ensure_bucket(
        ...
        if len(matches) > 1:
            raise VoicekitError("VK-DEP-007", detail="Railway bucket identity is ambiguous.")
        if matches:
            ...
            if state.bucket_id not in {None, bucket_id}:
                raise VoicekitError("VK-DEP-007", detail="Railway bucket id drifted.")
            checkpoint = state.checkpoint(bucket_id=bucket_id)
            self.store.save(checkpoint)
            return checkpoint
        if state.bucket_id is not None:
            raise VoicekitError("VK-DEP-007", detail="ledgered Railway bucket is missing.")
        result = self.runner.run(
            [
                "bucket",
                "create",
    ...
    def _ensure_domain(
        ...
        if len(matches) > 1:
            raise VoicekitError("VK-DEP-007", detail="Railway domain identity is ambiguous.")
        if matches:
            domain = _domain_text(matches[0])
            if not domain:
                raise VoicekitError("VK-DEP-007", detail="Railway domain is malformed.")
            checkpoint = state.checkpoint(
        ...
        if state.domain_id is not None or state.public_base is not None:
            raise VoicekitError("VK-DEP-007", detail="ledgered Railway domain is missing.")
        if domains:
            railway_domains = [
    ...
    def _sync_variables(
        ...
        references = {
            ...
            "VOICEKIT_OBJECT_BUCKET": _variable_reference(bucket, "BUCKET"),
            "VOICEKIT_OBJECT_ENDPOINT": _variable_reference(bucket, "ENDPOINT"),
        ...
        values = {
            **references,
            "VOICEKIT_PUBLIC_BASE": state.public_base,
            "VOICEKIT_OBJECT_PREFIX": "voicekit",
            "VOICEKIT_DEPLOY_TARGET": "railway",
        ...
        otlp_endpoint = environment.get("VOICEKIT_OTLP_ENDPOINT", "").strip()
        if otlp_endpoint:
            values["VOICEKIT_OTLP_ENDPOINT"] = otlp_endpoint
        self.runner.run(
            [
                "variable",
                "set",
        ...
        otlp_headers = environment.get("VOICEKIT_OTLP_HEADERS", "").strip()
        if otlp_headers:
            secret_values["VOICEKIT_OTLP_HEADERS"] = otlp_headers
            self.runner.run(
                [
                    "variable",
                    "set",
                    "VOICEKIT_OTLP_HEADERS_ENV=VOICEKIT_OTLP_HEADERS",
                    *self._context_args(state),
                    "--skip-deploys",
                    "--json",
                ],
    ...
    def _services(self, state: RailwayResourceState) -> list[dict[str, object]]:
        if state.project_id is None:
            raise VoicekitError("VK-DEP-007", detail="Railway project id is missing.")
        result = self.runner.run(
            [
                "service",
                "list",
    ...
    def _checkpoint_link(
        ...
        if project_id and state.project_id != project_id:
            raise VoicekitError("VK-DEP-007", detail="Railway link returned another project.")
...
# Generated by voicekit {__version__}. Secrets are supplied by Railway at runtime.
...
def _railway_config() -> str:
    payload = {
        ...
        "deploy": {
            "preDeployCommand": ("python -m voicekit.deploy.results_service --preflight-only"),
            "startCommand": "python -m voicekit.deploy.results_service",
