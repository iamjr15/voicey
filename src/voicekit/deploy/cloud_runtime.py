...
class CloudWorkerSettings:
    ...
    def from_environment(
        ...
        *,
        ...
        runtime = environment.get("VOICEKIT_RUNTIME", "")
        if runtime != expected_runtime:
            raise VoicekitError(
                "VK-DEP-008",
                detail=(
                    f"cloud image expects {expected_runtime!r}; "
                    f"VOICEKIT_RUNTIME selects {runtime!r}."
                ),
            )
        root_value = environment.get("VOICEKIT_PROJECT_ROOT", "/app/project")
        root = Path(root_value)
        if not root.is_absolute():
            raise VoicekitError(
                "VK-DEP-008",
                detail="VOICEKIT_PROJECT_ROOT must be an absolute path.",
            )
        relay_url = environment.get("VOICEKIT_RELAY_URL", "").rstrip("/")
        credential = RelayCredential.parse(_required(environment, "VOICEKIT_RELAY_CREDENTIAL"))
        ...
        if (
            ...
            raise VoicekitError("VK-DEP-008", detail="VOICEKIT_RELAY_URL is invalid.")
        return cls(
...
async def run_pipecat_cloud_session(
    ...
    *,
    ...
    if manifest.runtime != "pipecat" or agent.runtime != "pipecat":
        raise VoicekitError("VK-DEP-008", detail="deployed project is not a Pipecat agent.")
    ...
    try:
        ...
        transfer = _cloud_transfer(
            ...
            environment=values,
            scratch_root=Path("/tmp") / f"voicekit-{call.call_id}",
        )
        ...
        wait_task = asyncio.create_task(
            session.wait(),
            name=f"voicekit-cloud-{call.call_id}",
        )
        await runner.run()
        await wait_task
    finally:
...
async def run_livekit_cloud_worker(
    *,
    ...
    if manifest.runtime != "livekit" or agent.runtime != "livekit":
        raise VoicekitError("VK-DEP-008", detail="deployed project is not a LiveKit agent.")
    await validate_cloud_worker_startup(settings)

    from voicekit.runtimes.livekit import LiveKitHost, LiveKitHostSettings
    ...
    host = LiveKitHost(
        ...
        settings=LiveKitHostSettings(
            num_idle_processes=_integer(values, "VOICEKIT_CLOUD_IDLE_PROCESSES", default=2),
            drain_timeout_s=agent.limits.max_duration_s,
            session_end_timeout_s=float(agent.limits.max_duration_s),
            health_port=_integer(values, "VOICEKIT_CLOUD_HEALTH_PORT", default=8081),
...
def _load_project(settings: CloudWorkerSettings) -> tuple[ProjectManifest, Agent]:
    ...
    if not root.is_dir():
        raise VoicekitError("VK-DEP-008", detail="cloud project directory is unavailable.")
...
def _required(environment: Mapping[str, str], name: str) -> str:
    ...
    if not value:
        raise VoicekitError("VK-DEP-008", detail=f"cloud worker requires {name}.")
    return value
...
def _integer(environment: Mapping[str, str], name: str, *, default: int) -> int:
    try:
        ...
        raise VoicekitError("VK-DEP-008", detail=f"{name} must be an integer.") from exc
