...
from voicekit.config.catalog import DEFAULT_PROVIDER_CATALOG
...
_WORKER_SECRET_EXCLUSIONS = {
    ...
    "BUCKET_NAME",
    "DATABASE_URL",
    "VOICEKIT_ARTIFACT_BACKEND",
    "VOICEKIT_OBJECT_BUCKET",
    "VOICEKIT_OBJECT_ENDPOINT",
    "VOICEKIT_OBJECT_PREFIX",
    "VOICEKIT_OBJECT_REGION",
    "VOICEKIT_RELAY_PREVIOUS_CREDENTIAL",
    "VOICEKIT_RESULTS_PREVIOUS_SECRET",
    "VOICEKIT_RESULTS_SECRET",
    "VOICEKIT_STORAGE_BACKEND",
}
...
class CloudResourceState:
    ...
    def from_payload(cls, value: object) -> CloudResourceState:
        if not isinstance(value, dict):
            raise VoicekitError("VK-DEP-010", detail="cloud resource ledger is not an object.")
...
class CloudResourceStore:
    ...
    def __init__(self, project_root: Path, platform: CloudPlatform) -> None:
        self.path = project_root.resolve() / ".voicekit" / "deploy" / f"{platform}-resources.json"
    ...
    def load(self) -> CloudResourceState | None:
        ...
        try:
            ...
            raise VoicekitError(
                "VK-DEP-010", detail="cloud resource ledger cannot be read."
            ) from exc
...
class CloudArtifactGenerator:
    ...
    def generate(
        ...
        *,
        ...
        directory = self.project_root / ".voicekit" / "deploy" / platform
...
class PipecatCloudDeploymentManager:
    ...
    def __init__(
        ...
        *,
        ...
        if smoke_claim_timeout_s <= 0 or smoke_terminal_timeout_s <= 0 or smoke_poll_interval_s < 0:
            raise VoicekitError("VK-DEP-008", detail="cloud smoke timeouts are invalid.")
        self.project_root = project_root.resolve()
    ...
    def rollback_created(self, plan: PipecatCloudPlan) -> CloudResourceState:
        state = self.store.load()
        if state is None:
            raise VoicekitError("VK-DEP-010", detail="no Pipecat Cloud ledger exists.")
        state.validate(
            platform="pipecat-cloud",
            agent_name=plan.agent_name,
            account_scope=plan.organization,
        ...
        if not state.agent_created:
            raise VoicekitError(
                "VK-DEP-010",
                detail="adopted Pipecat Cloud agents cannot be deleted by voicekit.",
            )
        self.runner.run(
            [
                "cloud",
    ...
    def artifacts_directory(self) -> Path:
        return self.project_root / ".voicekit" / "deploy" / "pipecat-cloud" / "context"
...
class LiveKitCloudDeploymentManager:
    ...
    def rollback(self, plan: LiveKitCloudPlan) -> CloudResourceState:
        ...
        if state is None or state.agent_id is None:
            raise VoicekitError("VK-DEP-010", detail="no LiveKit Cloud ledger exists.")
        state.validate(
            platform="livekit-cloud",
            agent_name=plan.agent_name,
            account_scope=plan.project,
        ...
        cwd = self.project_root / ".voicekit" / "deploy" / "livekit-cloud" / "context"
...
def _validate_common(agent_name: str, region: str, relay_url: str) -> None:
    ...
    if (
        ...
        raise VoicekitError("VK-DEP-008", detail="cloud plan identity or relay URL is invalid.")
...
def _stage_wheel(source_value: Path | None, destination: Path) -> Path | None:
    if source_value is None:
        if __version__.endswith(".dev0"):
            raise VoicekitError(
                "VK-DEP-008",
                detail="unpublished builds require --engine-wheel voicekit-*.whl.",
            )
        return None
    ...
    if (
        ...
        raise VoicekitError("VK-DEP-008", detail="cloud engine wheel is invalid.")
...
def _cloud_dockerfile(
    ...
    *,
    ...
    package = (
        ...
        else f'"voicekit[{extras}]=={__version__}"'
    )
    wheel_copy = f"COPY {wheel.name} /tmp/{wheel.name}\n" if wheel is not None else ""
    entrypoint = (
        'COPY bot.py /app/bot.py\nCMD ["python", "/app/bot.py"]'
        if platform == "pipecat-cloud"
        else 'CMD ["python", "-m", "voicekit.deploy.cloud_runtime", "livekit"]'
    )
...
RUN uv pip install --python /opt/voicekit/bin/python {package} \\
    && if [ -s /tmp/project-requirements.txt ]; then \\
         uv pip install --python /opt/voicekit/bin/python -r /tmp/project-requirements.txt; \\
       fi
...
ENV PATH="/opt/voicekit/bin:$PATH" \\
    ...
    VOICEKIT_PROJECT_ROOT=/app/project
WORKDIR /app/project
USER 10001:10001
"""
...
from voicekit.deploy.cloud_runtime import run_pipecat_cloud_session
...
def _project_requirements(path: Path) -> str:
    try:
        ...
        raise VoicekitError("VK-DEP-008", detail="project pyproject.toml is invalid.") from exc
    if not isinstance(dependencies, list):
        raise VoicekitError("VK-DEP-008", detail="project dependencies are invalid.")
    ...
    return "".join(
        ...
        if not re.match(r"^\s*voicekit(?:\[|[<>=!~ @;]|$)", item, flags=re.IGNORECASE)
    )
...
def _worker_secrets(
    ...
    relay_value = combined.get("VOICEKIT_RELAY_CREDENTIAL", "").strip()
    ...
    values.update(
        {
            "VOICEKIT_DEPLOY_TARGET": f"{runtime}-cloud",
            "VOICEKIT_RELAY_CREDENTIAL": relay_value,
            "VOICEKIT_RELAY_URL": relay_url.rstrip("/"),
...
def _load_agent(project_root: Path, manifest: ProjectManifest) -> Agent:
    ...
    if not isinstance(value, Agent):
        raise VoicekitError("VK-DEP-008", detail="cloud project export is not an Agent.")
    return value
...
class _secret_file:
    ...
    def __enter__(self) -> Path:
        descriptor, name = tempfile.mkstemp(prefix="voicekit-cloud-secrets-", text=True)
        self.path = Path(name)
        try:
...
def _require_ready(output: str, *, platform: str) -> None:
    ...
    if not positive or negative:
        raise VoicekitError("VK-DEP-004", detail=f"{platform} did not report ready.")
    ...
    *,
    ...
    while True:
        ...
        if asyncio.get_running_loop().time() >= deadline:
            raise VoicekitError("VK-DEP-004", detail=failure)
...
def _livekit_agent_id(config: Path, output: str) -> str:
    if config.exists():
        try:
            ...
            raise VoicekitError("VK-DEP-010", detail="livekit.toml is invalid.") from exc
...
def _require_private_regular(path: Path) -> None:
    ...
    if mode & 0o077:
        raise VoicekitError("VK-SEC-001", detail=str(path))
