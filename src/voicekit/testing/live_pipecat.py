...
from voicekit.config.models import Voice
from voicekit.errors import VoicekitError
from voicekit.runtimes.pipecat.host import LongLivedRunner
from voicekit.runtimes.pipecat.providers import DefaultProviderFactory, ProviderFactory
...
from voicekit.testing.live import LiveCallEvidence, LiveCallPlan, LiveEnvironment
from voicekit.testing.models import LiveTestingConfig
from voicekit.tunnel import TunnelHandle, TunnelManager
...
class PipecatTwilioPstnBackend:
    ...
    def __init__(
        ...
        *,
        ...
        if live.runtime != "pipecat" or live.twilio_from_number is None:
            raise VoicekitError("VK-TST-003", detail="invalid Pipecat live-test configuration.")
        self._root = root
        self._config = config
        self._live = live
    ...
    async def run_call(self, plan: LiveCallPlan) -> LiveCallEvidence:
        if self._closed:
            raise VoicekitError("VK-TST-003", detail="Pipecat live caller is already closed.")
    ...
    async def _ensure_started(self) -> None:
        ...
        try:
            ...
            self._server_task = asyncio.create_task(
                server.serve(),
                name="voicekit-live-pstn-callback-server",
            )
    ...
    def _mount_routes(self) -> None:
        ...
        async def twilio_media(  # pyright: ignore[reportUnusedFunction]
            ...
            if not adapter.verify_request(_websocket_request(websocket)):
                await websocket.close(code=1008, reason="VK-TST-003")
                return
            ...
            try:
                ...
                with suppress(RuntimeError):
                    await websocket.close(code=1011, reason="VK-TST-003")
            finally:
                if state is not None:
    ...
    def _build_worker(
        ...
        async def on_caller_turn(  # pyright: ignore[reportUnusedFunction]
            ...
            if "thank you, goodbye" in message.content.casefold():
                ...
                _spawn(
                    state,
                    end_after_playout(),
                    name=f"voicekit-live-farewell-{state.plan.run_id}",
                )
        ...
        async def on_connected(  # pyright: ignore[reportUnusedFunction]
            ...
            _spawn(
                state,
                start_if_silent(),
                name=f"voicekit-live-opening-{state.plan.run_id}",
            )
    ...
    def _require_adapter(self) -> LiveTwilioAdapter:
        if self._adapter is None:
            raise VoicekitError("VK-TST-003", detail="live Twilio callback server is not ready.")
        return self._adapter

    def _require_tunnel(self) -> TunnelHandle:
        if self._tunnel is None:
            raise VoicekitError("VK-TST-003", detail="live Twilio tunnel is not ready.")
        return self._tunnel
