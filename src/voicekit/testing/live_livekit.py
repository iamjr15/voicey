...
class LiveKitSipPstnBackend:
    ...
    def __init__(
        ...
        *,
        ...
        if live.runtime != "livekit" or live.livekit_outbound_trunk_id is None:
            raise VoicekitError("VK-TST-003", detail="invalid LiveKit live-test configuration.")
        self._config = config
        self._live = live
    ...
    async def run_call(self, plan: LiveCallPlan) -> LiveCallEvidence:
        if self._closed:
            raise VoicekitError("VK-TST-003", detail="LiveKit live caller is already closed.")
        lkapi = self._api_client()
        room_name = f"voicekit-live-{plan.run_id}"[:128]
        participant_identity = f"voicekit-target-{plan.run_id}"[:128]
        ...
        try:
            ...
            token = (
                ...
                .with_identity(f"voicekit-sim-{plan.run_id}"[:128])
                .with_name("voicekit PSTN test caller")
            ...
            sip_info = await lkapi.sip.create_sip_participant(
                api.CreateSIPParticipantRequest(
                    ...
                    room_name=room_name,
                    participant_identity=participant_identity,
                    participant_name="voicekit target agent",
