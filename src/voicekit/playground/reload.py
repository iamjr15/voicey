...
class ReloadController:
    ...
    async def watch(self, stop_event: asyncio.Event) -> None:
        ...
        try:
            ...
            self._snapshot = ReloadSnapshot(
                ...
                state="error",
                message=f"VK-WEB-005: file watcher failed: {exc}",
            )
    ...
    async def apply(
        ...
        *,
        ...
        except (ImportError, AttributeError, OSError, VoicekitError, ValueError) as exc:
            self._snapshot = ReloadSnapshot(
                revision=self._snapshot.revision,
                state="error",
                message=f"VK-WEB-005: reload failed: {exc}",
            )
            return
        self._snapshot = ReloadSnapshot(
