...
from voicekit.storage.artifacts import validate_artifact_key

_BUCKET_PATTERN = re.compile(r"^[a-z0-9][a-z0-9.-]{1,61}[a-z0-9]$")
_DIGEST_METADATA_KEY = "voicekit-sha256"
...
class S3ArtifactStore:
    ...
    async def read(self, storage_key: str) -> bytes:
        """Read one object and verify its voicekit-owned digest metadata."""
        key = self._key(storage_key)
        ...
        try:
            ...
            raise VoicekitError("VK-ART-002", detail=f"{storage_key}: object read failed.") from exc
        finally:
            if body is not None:
                body.close()
    ...
    async def verify_round_trip(self, nonce: str) -> None:
        ...
        expected = hashlib.sha256(f"voicekit:{nonce}".encode()).digest()
...
def _validate_bucket(bucket: str) -> str:
    if (
        ...
        raise VoicekitError("VK-ART-003", detail=f"Invalid object-store bucket: {bucket!r}.")
    return bucket
