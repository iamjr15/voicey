...
from voicekit.storage.sqlite import MAX_DELIVERY_ATTEMPTS, RETRY_DELAYS_SECONDS
        ...
        async with self._pool.connection() as connection:
            try:
                async with connection.transaction():
                    ...
                    await connection.execute(
                        """
                        CREATE TABLE IF NOT EXISTS voicekit_schema_migrations (
                    ...
                    cursor = await connection.execute(
                        "SELECT version, name, checksum FROM voicekit_schema_migrations"
                    )
        ...
        async with self._pool.connection() as connection:
            try:
                cursor = await connection.execute(
                    ...
                    FROM voicekit_schema_migrations ORDER BY version
                    """
                )
                rows = list(await cursor.fetchall())
            except psycopg.Error as exc:
...
class PostgresRepository:
    ...
    def __init__(
        ...
        *,
        ...
        if not dsn or min_size < 0 or max_size < 1 or min_size > max_size or timeout_s <= 0:
            raise VoicekitError("VK-RES-008", detail="Postgres pool settings are invalid.")
    ...
    async def begin_call(
        ...
        *,
        ...
        try:
            ...
            raise VoicekitError("VK-RES-008", detail="Postgres begin_call failed.") from exc
        return CallLease(
        ...
        try:
            async with self._pool.connection() as connection, connection.transaction():
                ...
                if row["status"] != "active":
                    ...
                    if existing is None:
                        raise VoicekitError("VK-RES-007", detail=lease.call_id)
    ...
    async def list_calls(
        ...
        *,
        ...
        if not 1 <= limit <= 1000:
            raise VoicekitError("VK-OBS-005", detail=f"limit={limit}")
        async with self._pool.connection() as connection:
            if status is None:
                cursor = await connection.execute(
        ...
        try:
            async with self._pool.connection() as connection, connection.transaction():
                ...
                if relay_lease is not None:
                    _assert_owner(row, relay_lease, code="VK-REL-004")
                if row["status"] == "active":
                    raise VoicekitError(
                        "VK-RES-010",
    ...
    async def mark_recording_failed_fenced(self, lease: CallLease) -> None:
        ...
        if row is None:
            ...
            if existing is None:
                raise VoicekitError("VK-RES-010", detail=lease.call_id)
    ...
    async def claim_deliveries(
        ...
        *,
        ...
        try:
            ...
            raise VoicekitError("VK-RES-008", detail="Postgres delivery claim failed.") from exc
    ...
    async def fail_delivery(
        ...
        *,
        ...
        try:
            ...
            raise VoicekitError("VK-RES-008", detail="Postgres delivery failure failed.") from exc
    ...
    async def queue_retention(
        ...
        *,
        ...
        try:
            ...
            raise VoicekitError("VK-RES-008", detail="Postgres retention failed.") from exc
        return tuple(
    ...
    async def _fetch_one(
        ...
        try:
            ...
            raise VoicekitError("VK-RES-008", detail="Postgres query failed.") from exc
    ...
    async def _execute_count(
        ...
        try:
            ...
            raise VoicekitError("VK-RES-008", detail="Postgres mutation failed.") from exc
    ...
    async def _observation_write(
        ...
        *,
        ...
        try:
            ...
            raise VoicekitError("VK-OBS-002", detail="Postgres observation failed.") from exc
...
def _migration_sources() -> tuple[tuple[int, str, str, str], ...]:
    root = files("voicekit.storage").joinpath("migrations", "postgres")
...
def _assert_owner(
    ...
    *,
    ...
    if row["owner_id"] != lease.owner_id or int(row["generation"]) != lease.generation:
        raise VoicekitError(code, detail=lease.call_id)
...
def _utc(value: datetime | None) -> datetime:
    ...
    if current.tzinfo is None or current.utcoffset() is None:
        raise VoicekitError("VK-RES-008", detail="storage timestamp is timezone-naive.")
...
def _datetime(value: object) -> datetime:
    if not isinstance(value, datetime):
        raise VoicekitError("VK-RES-008", detail="Postgres timestamp is invalid.")
    return _utc(value)
