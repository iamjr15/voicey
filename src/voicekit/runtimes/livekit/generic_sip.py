...
class GenericSipConfig:
    """Operator-supplied desired state; voicekit never mutates the PBX/carrier."""
    ...
    def __post_init__(self) -> None:
        ...
        if port_text.isdecimal() and not 1 <= int(port_text) <= 65535:
            raise VoicekitError("VK-TEL-002", detail="generic SIP port is invalid.")
        ...
        if len(set(self.allowed_addresses)) != len(self.allowed_addresses):
            raise VoicekitError("VK-TEL-002", detail="generic SIP allowlist has duplicates.")
...
class GenericSipProvisioner:
    ...
    async def _ensure_inbound(self, config: GenericSipConfig) -> ManagedSipResource:
        ...
        if len(matching) > 1:
            raise VoicekitError("VK-TEL-006", detail="duplicate managed generic SIP trunks.")
    ...
    async def _ensure_dispatch(
        ...
        if len(matching) > 1:
            raise VoicekitError("VK-TEL-006", detail="duplicate managed generic SIP dispatch.")
        metadata = _metadata(config)
        if matching:
            item = matching[0]
