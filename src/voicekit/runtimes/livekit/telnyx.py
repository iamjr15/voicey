...
from voicekit.telephony.models import validate_e164
...
class TelnyxLiveKitSipConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if match.group(2) is not None and not 1 <= int(match.group(2)) <= 65535:
            raise VoicekitError("VK-TEL-002", detail="LiveKit SIP port is invalid.")
        ...
        if not self.auth_username or not self.auth_password:
            raise VoicekitError("VK-TEL-002", detail="Telnyx SIP credentials are required.")
        if len(self.auth_username) > 128 or len(self.auth_password) > 128:
            raise VoicekitError("VK-TEL-002", detail="Telnyx SIP credentials exceed 128 chars.")
        ...
        if self.anchorsite not in {
            ...
            raise VoicekitError("VK-TEL-002", detail="unsupported Telnyx anchor site.")
...
class TelnyxLiveKitSipProvisioner:
    ...
    async def _ensure_livekit_outbound(
        ...
        matching = [item for item in response.items if item.name == config.base_name]
        if len(matching) > 1:
            raise VoicekitError("VK-TEL-006", detail="duplicate managed LiveKit outbound trunks.")
        metadata = _managed_metadata(config)
...
class TelnyxSipHTTPBackend:
    ...
    def ensure_outbound_profile(self, *, name: str) -> ManagedSipResource:
        ...
        if len(matches) > 1:
            raise VoicekitError("VK-TEL-006", detail="duplicate managed outbound profiles.")
        if matches:
            item = matches[0]
    ...
    def ensure_connection(
        ...
        *,
        ...
        tag = f"voicekit:{config_fingerprint.removeprefix('sha256:')}"
        inbound = {
        ...
        if len(matches) > 1:
            raise VoicekitError("VK-TEL-006", detail="duplicate managed FQDN connections.")
        if matches:
            item = matches[0]
            if (
    ...
    def ensure_fqdn(
        ...
        *,
        ...
        if len(matches) > 1:
            raise VoicekitError("VK-TEL-006", detail="duplicate managed Telnyx FQDNs.")
        if matches:
            item = matches[0]
            try:
    ...
    def _list(
        ...
        *,
        ...
        if not isinstance(data, list):
            raise VoicekitError("VK-TEL-011", detail="Telnyx list response is malformed.")
        raw = cast("list[object]", data)
        if any(not isinstance(item, dict) for item in raw):
            raise VoicekitError("VK-TEL-011", detail="Telnyx list item is malformed.")
    ...
    def _data_object(document: dict[str, object]) -> dict[str, object]:
        ...
        if not isinstance(data, dict):
            raise VoicekitError("VK-TEL-011", detail="Telnyx response data is malformed.")
...
def _provider_id(value: object, field_name: str) -> str:
    ...
    if not normalized or len(normalized) > 512 or any(char.isspace() for char in normalized):
        raise VoicekitError("VK-TEL-006", detail=f"Telnyx returned an invalid {field_name}.")
    return normalized
