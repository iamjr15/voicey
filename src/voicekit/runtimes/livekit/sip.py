...
from voicekit.storage.models import EndedReason, RecordingReady
...
class TwilioLiveKitSipConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not re.fullmatch(
            ...
            raise VoicekitError("VK-TEL-002", detail="Twilio SIP domain name is invalid.")
        ...
        if not self.auth_username or not self.auth_password:
            raise VoicekitError("VK-TEL-002", detail="SIP username and password are required.")
...
class TwilioElasticSipBackend:
    ...
    def ensure_trunk(self, *, name: str, domain_name: str) -> ManagedSipResource:
        ...
        if len(matches) > 1:
            raise VoicekitError("VK-TEL-006", detail="duplicate managed Twilio SIP trunks.")
        if matches:
            item = matches[0]
    ...
    def completed_trunk_recording(
        ...
        if not _TWILIO_CALL_SID.fullmatch(call_sid):
            raise VoicekitError("VK-TEL-009", detail="invalid Twilio CallSid for recording.")
        items = cast(
            "list[Any]",
    ...
    def ensure_credential(
        ...
        *,
        ...
        if len(matches) > 1:
            raise VoicekitError("VK-TEL-006", detail="duplicate managed Twilio credentials.")
        if matches:
            return ManagedSipResource(
                "twilio_credential",
                str(matches[0].sid),
    ...
    def ensure_credential_binding(
        ...
        *,
        ...
        matches = [
            ...
            if str(item.credential_list_sid) == credential_list_sid
        ]
        if matches:
            return ManagedSipResource(
    ...
    def restore_number(self, snapshot: dict[str, object]) -> None:
        ...
        if current != expected:
            raise VoicekitError("VK-TEL-006", detail="Twilio route rollback did not compare equal.")
