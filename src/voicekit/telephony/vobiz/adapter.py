...
class VobizAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        if not _ACCOUNT_ID.fullmatch(self.auth_id):
            raise VoicekitError("VK-TEL-002", detail="VOBIZ_AUTH_ID is missing or invalid.")
        ...
        if (
            ...
            raise VoicekitError("VK-TEL-002", detail="Vobiz base URL must be normalized HTTPS.")
        if not re.fullmatch(r"^[A-Z]{3}$", currency):
            raise VoicekitError("VK-TEL-002", detail="Vobiz currency must be a 3-letter code.")
    ...
    def buy_number(self, country: str, area: str | None = None) -> NumberInfo:
        ...
        if not isinstance(raw, dict):
            raise VoicekitError("VK-TEL-011", detail="Vobiz purchase response lacks a number.")
    ...
    async def download_recording(
        ...
        *,
        ...
        if (
            ...
            raise VoicekitError("VK-TEL-009", detail="Vobiz recording URL is not safe HTTPS.")
        if max_bytes <= 0:
    ...
    def answer_response(self, target: RuntimeTarget) -> str:
        ...
        if len(encoded.encode()) > 4096:
            raise VoicekitError("VK-TEL-002", detail="inline VobizXML exceeds the 4KB limit.")
        return encoded
    ...
    def parse_event(self, request: TelephonyRequest) -> CallEvent:
        ...
        if recording_url or recording_id:
            if not recording_url or not recording_id:
                raise VoicekitError("VK-TEL-008", detail="Vobiz recording callback is incomplete.")
            return CallEvent(
        ...
        *,
        ...
        while True:
            ...
            if not isinstance(raw_items, list):
                raise VoicekitError("VK-TEL-011", detail="Vobiz number list is malformed.")
    ...
    def _ensure_application(self, target: PipecatTarget) -> tuple[str, bool]:
        ...
        if not isinstance(raw_apps, list):
            raise VoicekitError("VK-TEL-011", detail="Vobiz application list is malformed.")
        ...
        if len(matches) > 1:
            raise VoicekitError("VK-TEL-006", detail="duplicate managed Vobiz applications.")
        if matches:
            app = matches[0]
            if (
    ...
    def _attach_application(self, number: str, app_id: str | None) -> None:
        if app_id is None:
            raise VoicekitError("VK-TEL-006", detail="Vobiz application id is absent.")
        self._request(
            "POST",
    ...
    def _assign_trunk(self, number: str, trunk_id: str | None) -> None:
        if trunk_id is None:
            raise VoicekitError("VK-TEL-006", detail="Vobiz trunk id is absent.")
        self._request(
            "POST",
...
def _direction(value: str) -> Literal["inbound", "outbound"] | None:
    ...
    if normalized in {"outgoing", "outbound"}:
        return "outbound"
    raise VoicekitError("VK-TEL-008", detail="Vobiz callback direction is invalid.")
...
def _form_value(form: object | None, name: str, *, required: bool = True) -> str:
    ...
    if value is None or value == "":
        if required:
            raise VoicekitError("VK-TEL-008", detail=f"Vobiz callback lacks {name}.")
        return ""
    if isinstance(value, (str, int, float, bool)):
        return str(value)
    raise VoicekitError("VK-TEL-008", detail=f"Vobiz callback {name} is malformed.")


def _provider_id(value: str, *, field_name: str) -> str:
    if not _PROVIDER_ID.fullmatch(value):
        raise VoicekitError("VK-TEL-008", detail=f"invalid Vobiz {field_name}.")
    return value
...
def _optional(value: object) -> str | None:
    ...
    if isinstance(value, (str, int, float)):
        return str(value)
    raise VoicekitError("VK-TEL-011", detail="Vobiz response field has an invalid scalar type.")
...
def _integer(value: object, *, default: int) -> int:
    ...
    try:
        ...
        raise VoicekitError("VK-TEL-011", detail="Vobiz pagination total is invalid.") from exc
