...
class TelnyxAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        if not self._api_key:
            raise VoicekitError("VK-TEL-002", detail="TELNYX_API_KEY is required.")
        ...
        if not base_url.startswith("https://"):
            raise VoicekitError("VK-TEL-002", detail="Telnyx base URL must be HTTPS.")
    ...
    async def download_recording(
        ...
        *,
        ...
        if (
            ...
            raise VoicekitError("VK-TEL-009", detail="Telnyx recording URL is not safe HTTPS.")
        if max_bytes <= 0:
    ...
    def answer_response(self, target: RuntimeTarget) -> str:
        ...
        if len(encoded.encode()) > 4096:
            raise VoicekitError("VK-TEL-002", detail="inline TeXML exceeds the 4KB limit.")
        return encoded
    ...
    def _parse_json_event(self, request: TelephonyRequest) -> CallEvent:
        try:
            ...
            raise VoicekitError("VK-TEL-008", detail="Telnyx callback is not valid JSON.") from exc
        if not isinstance(loaded, dict):
            raise VoicekitError("VK-TEL-008", detail="Telnyx callback envelope is not an object.")
        document = cast("dict[str, object]", loaded)
        raw_data = document.get("data", document)
        if not isinstance(raw_data, dict):
            raise VoicekitError("VK-TEL-008", detail="Telnyx callback lacks data.")
        data = cast("dict[str, object]", raw_data)
        raw_payload = data.get("payload")
        if not isinstance(raw_payload, dict):
            raise VoicekitError("VK-TEL-008", detail="Telnyx callback lacks payload.")
    ...
    def _data_object(document: dict[str, object]) -> dict[str, object]:
        ...
        if not isinstance(data, dict):
            raise VoicekitError("VK-TEL-011", detail="Telnyx response lacks an object data field.")
    ...
    def _data_list(document: dict[str, object]) -> list[dict[str, object]]:
        ...
        if not isinstance(data, list):
            raise VoicekitError("VK-TEL-011", detail="Telnyx response lacks an array data field.")
        raw_items = cast("list[object]", data)
        if any(not isinstance(item, dict) for item in raw_items):
            raise VoicekitError("VK-TEL-011", detail="Telnyx response has a malformed data item.")
...
def _json_status_event(
    ...
    if event_type in {"streaming.failed", "call.siprec.failed"}:
        return "failed", "carrier_error"
    raise VoicekitError("VK-TEL-008", detail=f"unknown Telnyx event type {event_type!r}.")
...
def _texml_status_event(
    ...
    if status in {"busy", "no-answer", "failed", "canceled"}:
        return "failed", "carrier_error"
    raise VoicekitError("VK-TEL-008", detail=f"unknown Telnyx CallStatus {status!r}.")
...
def _intent_from_payload(payload: dict[str, object]) -> str | None:
    ...
    try:
        ...
        raise VoicekitError("VK-TEL-008", detail="Telnyx client_state is invalid.") from exc
...
def _direction(value: object) -> Literal["inbound", "outbound"] | None:
    ...
    if normalized in {"outgoing", "outbound"}:
        return "outbound"
    raise VoicekitError("VK-TEL-008", detail="Telnyx callback direction is invalid.")
...
def _command_id(action: str, call_id: str) -> str:
    return str(uuid.uuid5(uuid.NAMESPACE_URL, f"voicekit:telnyx:{action}:{call_id}"))


def _validate_provider_id(value: str, *, field_name: str) -> str:
    if not _PROVIDER_ID.fullmatch(value):
        raise VoicekitError("VK-TEL-008", detail=f"invalid Telnyx {field_name}.")
    return value
