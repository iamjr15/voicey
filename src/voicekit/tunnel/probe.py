...
class TunnelProbe:
        ...
        async def tunnel_probe(websocket: WebSocket) -> None:  # pyright: ignore[reportUnusedFunction]
            ...
            try:
                ...
                if not secrets.compare_digest(received, self.token):
                    await websocket.close(code=1008, reason="VK-TUN-004")
                    return
    ...
    async def verify(
        ...
        *,
        ...
        if timeout_s <= 0:
            raise VoicekitError("VK-TUN-002", detail="tunnel probe timeout must be positive.")
...
def _websocket_url(
    ...
    *,
    ...
    if (
        ...
        raise VoicekitError("VK-TUN-002", detail="invalid tunnel probe public URL.")
    try:
        port = parsed.port
    except ValueError as exc:
        raise VoicekitError("VK-TUN-002", detail="tunnel probe URL has an invalid port.") from exc
