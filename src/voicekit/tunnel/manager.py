...
class NgrokListener(Protocol):
    """Installed ngrok listener methods used by voicekit."""
...
class TunnelManager:
    ...
    def resolve(self, preference: TunnelPreference = "auto") -> TunnelProvider:
        ...
        if preference == "cloudflared":
            return "cloudflared"
        if preference != "auto":
            raise VoicekitError("VK-TUN-002", detail=f"unsupported tunnel {preference!r}.")
...
async def _read_cloudflared_url(
    ...
    *,
    ...
    readers = [
        asyncio.create_task(_pump_lines(stream, queue), name=f"voicekit-tunnel-log-{index}")
            ...
            while closed_streams < len(readers):
                ...
                if match is not None:
                    drainer = asyncio.create_task(
                        ...
                        name="voicekit-tunnel-log-drain",
                    )
                    retain_readers = True
                    return (
...
def _validate_open_options(
    *,
    ...
    if not 1 <= port <= 65535:
        raise VoicekitError("VK-TUN-002", detail="local tunnel port must be 1 through 65535.")
    if preference not in {"auto", "ngrok", "cloudflared", "url"}:
        raise VoicekitError("VK-TUN-002", detail=f"unsupported tunnel {preference!r}.")
    if preference == "url" and public_url is None:
        raise VoicekitError("VK-TUN-002", detail="--tunnel url requires an HTTPS public URL.")
    ...
    if startup_timeout_s <= 0 or shutdown_timeout_s <= 0:
        raise VoicekitError("VK-TUN-002", detail="tunnel timeouts must be positive.")
...
def _validate_public_url(value: str) -> str:
    ...
    try:
        ...
        raise VoicekitError("VK-TUN-002", detail="tunnel public URL has an invalid port.") from exc
    if (
        parsed.scheme != "https"
...
def _import_ngrok() -> ModuleType:
    try:
        ...
        raise VoicekitError(
            "VK-TUN-001",
            detail='install the ngrok SDK with `uv pip install "voicekit[tunnel]"`.',
        ) from exc
