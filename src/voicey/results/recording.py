"""Runtime-neutral carrier recording ingestion and protected artifact access."""
...
from voicey.storage.models import RecordingReady
...
class PipecatRecordingHandler:
    ...
    async def handle_twilio(self, event: CallEvent) -> None:
        ...
        if event.type != "recording_ready" or event.recording_sid is None:
            raise VoiceyError("VY-TEL-009", detail="invalid Twilio recording callback.")
        if self.twilio is None:
            raise VoiceyError("VY-TEL-009", detail="Twilio recording adapter is unavailable.")
    ...
    async def handle_telnyx(self, event: CallEvent) -> None:
        ...
        if (
            ...
            raise VoiceyError("VY-TEL-009", detail="invalid Telnyx recording callback.")
        if self.telnyx is None:
            raise VoiceyError("VY-TEL-009", detail="Telnyx recording adapter is unavailable.")
        pending = await self._pending(event.provider_call_id)
        if pending is None:
            return
        recording_id, storage_key = pending
    ...
    async def handle_vobiz(self, event: CallEvent) -> None:
        ...
        if (
            ...
            raise VoiceyError("VY-TEL-009", detail="invalid Vobiz recording callback.")
        if self.vobiz is None:
            raise VoiceyError("VY-TEL-009", detail="Vobiz recording adapter is unavailable.")
        pending = await self._pending(event.provider_call_id)
        if pending is None:
            return
        recording_id, storage_key = pending
    ...
    async def handle_plivo(self, event: CallEvent) -> None:
        ...
        if (
            ...
            raise VoiceyError("VY-TEL-009", detail="invalid Plivo recording callback.")
        if self.plivo is None:
            raise VoiceyError("VY-TEL-009", detail="Plivo recording adapter is unavailable.")
        pending = await self._pending(event.provider_call_id)
        if pending is None:
            return
        recording_id, storage_key = pending
    ...
    async def read(self, recording_id: str, authorization: str | None) -> bytes:
        ...
        if not any(secrets.compare_digest(supplied, candidate) for candidate in allowed):
            raise VoiceyError("VY-WEB-004", detail="recording bearer authorization failed.")
    ...
    async def _pending(self, call_id: str) -> tuple[str, str] | None:
        ...
        if snapshot.status == "failed":
            raise VoiceyError("VY-RES-010", detail=f"{call_id} recording already failed.")
