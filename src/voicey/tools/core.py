...
_METADATA_ATTRIBUTE = "__voicey_tool__"
...
class ToolDecorator:
    ...
    def __call__(
        ...
        *,
        ...
        """Mark a plain typed function as a voicey tool without wrapping it."""
...
def metadata_from_callable(
    ...
    *,
    ...
    try:
        ...
        raise VoiceyError("VY-TOL-002", detail=f"{name}: {exc}") from exc
...
def get_tool_metadata(function: Callable[..., Any]) -> ToolMetadata:
    ...
    if not isinstance(metadata, ToolMetadata):
        ...
        raise VoiceyError("VY-TOL-001", detail=detail)
    return metadata
...
def validate_tool_name(name: str) -> None:
    ...
    if not _TOOL_NAME.fullmatch(name):
        raise VoiceyError("VY-TOL-002", detail=f"invalid tool name: {name!r}.")
