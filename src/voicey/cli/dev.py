...
from voicey.tunnel import TunnelManager, TunnelPreference, TunnelProbe
    ...
    *,
    ...
    with _project_environment(context.environment), _project_import_path(context.root):
        ...
        try:
            ...
            repository_path = context.root / ".voicey" / "calls.sqlite3"
            async with SQLiteRepository(repository_path) as repository:
                ...
                recording_handler = (
                    PipecatRecordingHandler(
                        ...
                        artifact_store=LocalArtifactStore(context.root / ".voicey" / "artifacts"),
                ...
                with embedded_frontend() as frontend:
                    ...
                    public_task = asyncio.create_task(
                        public_server.serve(),
                        name="voicey-public-listener",
                    )
                    admin_task = asyncio.create_task(
                        admin_server.serve(),
                        name="voicey-admin-listener",
                    )
...
async def _run_livekit_dev(
    ...
    *,
    ...
    if agent.runtime != "livekit":
        raise VoiceyError(
            "VY-CLI-007",
            detail="voicey.jsonc selects LiveKit but agent.py exports another runtime.",
        )
    if port > 65533:
        raise VoiceyError(
            "VY-CLI-010",
    ...
    public_app = FastAPI(
        title=f"voicey livekit signaling:{agent.name}",
        docs_url=None,
        redoc_url=None,
        openapi_url=None,
    )
    ...
    async with SQLiteRepository(repository_path) as repository:
        with embedded_frontend() as frontend:
            ...
            public_task = asyncio.create_task(
                public_server.serve(),
                name="voicey-livekit-token-listener",
            )
            admin_task = asyncio.create_task(
                admin_server.serve(),
                name="voicey-admin-listener",
...
async def _provision_livekit_phone(
    ...
    *,
    ...
    ledger = TelephonyLedger(context.root / ".voicey" / "telephony.sqlite3")
    try:
        if agent.phone.provider == "twilio":
            ...
            allowed = tuple(
                value.strip()
                for value in context.environment.get("VOICEY_SIP_ALLOWED_ADDRESSES", "").split(",")
                if value.strip()
            )
...
def _required_environment(environment: dict[str, str], name: str) -> str:
    ...
    if not value:
        raise VoiceyError(
            "VY-CLI-004",
            detail=f"{name} is required. Run `voicey keys add livekit` and retry.",
        )
    return value
    ...
    *,
    ...
    async def wait() -> None:
        while not server.started:
            if task.done():
                await task
                raise VoiceyError("VY-CLI-009", detail="development server exited early.")
