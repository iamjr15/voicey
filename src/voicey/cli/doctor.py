...
from voicey.cli.context import ProjectContext, require_manifest
...
from voicey.compatibility import inspect_runtime_compatibility
...
from voicey.telephony.models import PipecatTarget
        ...
        *,
        ...
        try:
            ...
            return (
                ...
                ("Check project permissions and network, then rerun `voicey doctor`.",),
            )
        finally:
        ...
        return (
            ...
            (
                "Run `voicey dev --phone` for a temporary provisioned route "
...
class Doctor:
    ...
    def __init__(
        ...
        *,
        ...
        if not 1 <= port <= 65535:
            raise VoiceyError("VY-CLI-006", detail="doctor port must be from 1 through 65535.")
    ...
    def apply_safe_fixes(self) -> tuple[str, ...]:
        ...
        if not values.get("VOICEY_WEBHOOK_SECRET") and not self.context.environment.get(
            "VOICEY_WEBHOOK_SECRET"
        ):
            env_store.update({"VOICEY_WEBHOOK_SECRET": encode_secret(secrets.token_bytes(32))})
            self.context.environment.update(env_store.read())
            fixed.append("generated VOICEY_WEBHOOK_SECRET")
        data_dir = self.context.root / ".voicey"
        data_dir.mkdir(mode=0o700, parents=True, exist_ok=True)
        data_dir.chmod(0o700)
        fixed.append("secured .voicey directory")
        return tuple(fixed)
    ...
    async def _keys(self) -> DoctorCheck:
        ...
        secret = self.context.environment.get("VOICEY_WEBHOOK_SECRET")
        if not secret:
            issues.append("VOICEY_WEBHOOK_SECRET is missing.")
            advice.append("Run `voicey doctor --fix`.")
        else:
            try:
                WebhookSigner(secret)
            except VoiceyError:
                issues.append("VOICEY_WEBHOOK_SECRET is not a valid whsec_ value.")
                advice.append("Run `voicey doctor --fix` after removing the invalid value.")
    ...
    async def _tunnel(self) -> DoctorCheck:
        ...
        public_url = self.context.environment.get("VOICEY_PUBLIC_URL")
        if not public_url:
            return _result(
                "tunnel",
                "Public tunnel reachability",
                ["VOICEY_PUBLIC_URL is absent."],
                ["Run `voicey dev --phone` to create and probe a tunnel."],
            )
        try:
        ...
        return _result(
            ...
            "Public tunnel reachability",
            issues,
            ["Restart `voicey dev --phone` and use its current public URL."] if issues else [],
        )
    ...
    async def _livekit(self) -> DoctorCheck:
        ...
        if "phone" in self.manifest.channels:
            if self.manifest.carriers == ["telnyx"]:
                ...
                sip_names = (
                    "VOICEY_LIVEKIT_SIP_URI",
                    "VOICEY_TWILIO_SIP_DOMAIN",
                    "VOICEY_TWILIO_SIP_USERNAME",
                    "VOICEY_TWILIO_SIP_PASSWORD",
                )
            ...
            if missing:
                ...
                advice.append(f"Add the {carrier_label} SIP values, then rerun `voicey doctor`.")
                ...
                sip_issues, sip_advice = await self.livekit_sip_inspector.inspect(
                    self.context.environment,
                    expected_name=f"voicey-{self.manifest.project_name}-{digits}",
                )
        ...
        if any(not self.context.environment.get(name) for name in LIVEKIT_ENV_VARS):
            advice.append("Run `voicey keys add livekit`.")
        return _result(
            "livekit",
            "LiveKit project, SIP trunk, and dispatch",
            issues,
    ...
    async def _receiver(self) -> DoctorCheck:
        ...
        except (httpx.HTTPError, VoiceyError):
    ...
    async def _dlq(self) -> DoctorCheck:
        path = self.context.root / ".voicey" / "calls.sqlite3"
        if not path.exists():
        ...
        advice = ["Run `voicey calls list --undelivered`."] if issues else []
...
def _twilio_check(context: ProjectContext, manifest: ProjectManifest) -> DoctorCheck:
    ...
    adapter = TwilioAdapter(
        ...
        expected_public_base=context.environment.get("VOICEY_PUBLIC_URL"),
    )
    try:
        ...
        if len(selected) != 1:
            ...
            advice.append("Select an owned E.164 number with `voicey init --resume`.")
        ...
        public_url = context.environment.get("VOICEY_PUBLIC_URL")
        if public_url:
            ...
            if route["voice_url"] != expected:
                ...
                advice.append("Run `voicey numbers point --yes` or `voicey dev --phone`.")
        else:
            issues.append("VOICEY_PUBLIC_URL is absent, so the live Twilio route cannot be diffed.")
            advice.append("Run `voicey dev --phone`.")
    finally:
...
def _telnyx_check(context: ProjectContext, manifest: ProjectManifest) -> DoctorCheck:
    ...
    if manifest.carriers != ["telnyx"] or manifest.phone_number is None:
        return _result(
            ...
            ["Resume init with Twilio or Telnyx, then rerun `voicey doctor`."],
        )
    ...
    try:
        ...
        if len(selected) != 1:
            ...
            if route.get("connection_id") != expected:
                ...
                advice.append("Run `voicey dev --phone` for a temporary conflict-safe route.")
        advice.append(
...
def _vobiz_check(context: ProjectContext, manifest: ProjectManifest) -> DoctorCheck:
    ...
    if manifest.carriers != ["vobiz"] or manifest.phone_number is None:
        return _result(
            ...
            "Carrier account, funding, KYC, and inbound route",
            ["The selected carrier is not available in this certification phase."],
            ["Resume init with Twilio, Telnyx, or Vobiz, then rerun `voicey doctor`."],
        )
    ...
    if missing:
        return _result(
            ...
            ["Add the Vobiz Auth ID and Auth Token, then rerun `voicey doctor`."],
        )
    issues: list[str] = []
    advice: list[str] = []
...
def _plivo_check(context: ProjectContext, manifest: ProjectManifest) -> DoctorCheck:
    ...
    if missing:
        return _result(
            ...
            ["Run `voicey keys add plivo`, then rerun `voicey doctor`."],
        )
    issues: list[str] = []
    advice: list[str] = []
    ...
    try:
        ...
        if len(selected) != 1:
            ...
            if not route.get("app_id"):
                ...
                advice.append("Run `voicey dev --phone` for a conflict-safe temporary route.")
        advice.append(
...
def _generic_sip_check(context: ProjectContext, manifest: ProjectManifest) -> DoctorCheck:
    ...
    if missing:
        ...
        advice.append("Run `voicey keys add sip`, then rerun `voicey doctor`.")
    advice.append(
...
def _env_check(context: ProjectContext) -> DoctorCheck:
    ...
    return _result(
        ...
        ".env and .env.example agreement",
        issues,
        ["Run the matching `voicey keys add <provider>` command."] if issues else [],
    )
