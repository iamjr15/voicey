...
class ScaffoldWriter:
    ...
    def write(
        ...
        if gitignore.is_symlink():
            raise VoiceyError("VY-SEC-002", detail=str(gitignore))
        if gitignore.exists():
            try:
        ...
        try:
            ...
            ManifestStore(project_dir / "voicey.jsonc").save(manifest)
        except Exception:
...
def _render(scaffold: ScratchScaffold) -> dict[str, str]:
    ...
    behavior = (
        ...
        '        transfer_number=environ.get("VOICEY_TRANSFER_NUMBER"),\n'
        "    ),\n"
        if is_recipe
        else ""
    )
    agent = f'''"""Generated voicey agent. Customize the TODO-marked integration points."""

{environment_import}from voicey import Agent{behavior_import}, Models, Phone, Results, Web
...
{behavior}    results=Results(
        webhook="https://example.invalid/voicey-results",  # TODO: receiver
...
"""
    ...
    if is_recipe:
        env_names.add("VOICEY_TRANSFER_NUMBER")
    env_example = "\n".join(f"{name}=" for name in sorted(env_names)) + "\n"
    readme = f"""# {scaffold.project_name}

{description}

Generated as a native {runtime_label}. Start with `voicey doctor`, then run
`voicey dev`. Search for `TODO` before connecting production systems.
"""
    rendered = {
        "agent.py": agent,
        "flow.py": flow,
