...
class ProviderKeyValidator:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_s <= 0:
            raise VoiceyError("VY-CLI-004", detail="key validation timeout must be positive.")
        self._client = client
    ...
    async def validate(
        ...
        if missing:
            return KeyCheck(
                ...
                fix=f"Run `voicey keys add {provider}`.",
            )
        if identifier == "sip":
        ...
        *,
        ...
        if response.status_code == 401:
            return KeyCheck(
                ...
                fix=f"Paste a current {provider} key with `voicey keys add {provider}`.",
            )
        return KeyCheck(
            provider=provider,
            env_names=entry.key_env_vars,
    ...
    async def validate(self, values: Mapping[str, str]) -> KeyCheck:
        ...
        if missing:
            return KeyCheck(
                ...
                status="missing",
                detail=f"{', '.join(missing)} is missing.",
                fix="Run `voicey keys add livekit`.",
            )
        try:
            ...
            return KeyCheck(
                ...
                fix='Install with `uv pip install "voicey[livekit]"`, then retry.',
            )
        ...
        try:
            ...
            return KeyCheck(
                ...
                fix="Replace the project credentials with `voicey keys add livekit`.",
            )
            ...
            return KeyCheck(
                ...
                fix=(
                    "Replace the project credentials with `voicey keys add livekit`."
                    if invalid
...
def _validate_generic_sip(
    ...
    address = values["VOICEY_SIP_ADDRESS"].casefold()
    username = values["VOICEY_SIP_USERNAME"]
    password = values["VOICEY_SIP_PASSWORD"]
    transport = values["VOICEY_SIP_TRANSPORT"].casefold()
    encryption = values["VOICEY_SIP_MEDIA_ENCRYPTION"].casefold()
    valid_address = bool(
        re.fullmatch(
    ...
    return KeyCheck(
        ...
        fix=(
            "No action required."
            if valid
            else "Run `voicey keys add sip`; use host[:port], udp/tcp/tls, and "
