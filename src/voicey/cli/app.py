"""Production command rail for voicey projects."""
...
from voicey.capabilities import DEFAULT_CAPABILITIES
...
from voicey.cli.doctor import Doctor, DoctorCheck
from voicey.cli.environment import EnvFileStore, ensure_env_ignored
...
from voicey.cli.wizard import InitOptions, InitWizard
from voicey.config.catalog import DEFAULT_PROVIDER_CATALOG, ProviderCatalogEntry
from voicey.config.manifest import ManifestStore, RecipeSelection
from voicey.deploy import (
...
from voicey.recipes.source import install_recipe
...
from voicey.testing.reporting import result_json, write_junit
from voicey.testing.runner import run_project_tests
from voicey.tunnel import TunnelPreference
from voicey.upgrade import UpgradeManager
...
def main(
    ...
    version: Annotated[
        bool,
        typer.Option("--version", help="Print the installed voicey version."),
    ] = False,
    ...
    """Show project status or dispatch a voicey command."""
    if version:
    ...
    if context.manifest is not None:
        ...
        console.print("[bold]voicey[/bold] setup is incomplete.")
    else:
        console.print("[bold]voicey[/bold] is installed.")
...
def init_command(
    ...
    async def operation() -> None:
        ...
        console.print(f"Next: cd {result.project_dir} && voicey dev")
...
def dev_command(
    ...
    if tunnel not in {"auto", "cloudflared", "ngrok", "url"}:
        _fail(VoiceyError("VY-CLI-010", detail=f"unknown tunnel {tunnel!r}."))

    async def operation() -> None:
        from voicey.cli.dev import run_dev
        ...
        console.print("Next: voicey calls list")
...
def call_command(
    ...
    def operation() -> None:
        ...
        if manifest.phone_number is None:
            raise VoiceyError("VY-CLI-007", detail="this project has no phone number.")
        _confirm(
            f"Place a paid call from {manifest.phone_number} to {e164}?",
            yes=yes,
        )
        target_url = public_url or context.environment.get("VOICEY_PUBLIC_URL")
        if not target_url:
            raise VoiceyError(
                "VY-CLI-007",
                detail="start `voicey dev --phone` or pass --url first.",
            )
...
def doctor_command(
    ...
    async def operation() -> None:
        ...
        payload = {
            ...
            "next_step": "voicey dev" if report.ok else "voicey doctor",
        }
        if json_output:
            _json(payload)
        else:
...
def keys_list(
    ...
    async def operation() -> None:
        ...
        _rows_or_table(
            ...
            next_command="voicey keys validate",
        )
...
def keys_add(
    ...
    async def operation() -> None:
        ...
        console.print("Next: voicey doctor")
...
def recipes_list(
    ...
    def operation() -> None:
        ...
        _rows_or_table(
            ...
            json_output=json_output,
            next_command="voicey init --recipe appointment-booking",
        )
...
def recipes_add(
    ...
    def operation() -> None:
        ...
        try:
            ManifestStore(context.root / "voicey.jsonc").save(updated)
        except Exception:
...
def numbers_buy(
    ...
    def operation() -> None:
        ...
        console.print(f"Next: voicey numbers point {number.number} --yes")
...
def numbers_release(
    ...
    def operation() -> None:
        ...
        console.print("Next: voicey numbers list")
...
def numbers_point(
    ...
    def operation() -> None:
        ...
        if selected_number is None or target_url is None:
            raise VoiceyError(
                "VY-CLI-007",
                detail="an owned number and --url/VOICEY_PUBLIC_URL are required.",
            )
        ...
        console.print(f"Next: voicey numbers restore {token.token} --yes")
...
def calls_list(
    ...
    async def operation() -> None:
        path = _context().root / ".voicey" / "calls.sqlite3"
        if not path.exists():
            rows: list[dict[str, object]] = []
        else:
        ...
        _rows_or_table(
            ...
            columns=columns,
            json_output=json_output,
            next_command="voicey calls show <call-id>",
        )
...
def calls_show(
    ...
    async def operation() -> None:
        ...
        if json_output:
            _json({"call": payload, "next_step": "voicey calls list"})
        else:
...
def calls_redeliver(
    ...
    async def operation() -> None:
        ...
        path = _context().root / ".voicey" / "calls.sqlite3"
        if not path.exists():
            raise VoiceyError("VY-RES-009", detail=identifier)
        async with SQLiteRepository(path) as repository:
            try:
                event = await repository.get_event(identifier)
            except VoiceyError as exc:
                if exc.code != "VY-RES-009":
                    raise
        ...
        console.print("Next: voicey calls list --undelivered")
    ...
    async def operation() -> None:
        ...
        if normalized_report == "json":
            ...
            if normalized_report == "junit":
                junit = write_junit(
                    suite,
                    context.root / ".voicey" / "test-results.xml",
                )
...
def deploy_command(
    ...
    engine_wheel: Annotated[
        Path | None,
        typer.Option("--engine-wheel", help="Local unpublished voicey wheel."),
    ] = None,
    json_output: Annotated[
        bool,
    ...
    async def operation() -> None:
        ...
        if to_number is not None and smoke_url is None:
            raise VoiceyError("VY-CLI-010", detail="--to requires --smoke URL.")
        context = _context()
        manifest = require_manifest(context)
        ...
        await asyncio.to_thread(
            ManifestStore(context.root / "voicey.jsonc").save,
            updated,
        )
        ...
        if smoke_url is not None:
            ...
            destination = to_number or context.environment.get("VOICEY_SMOKE_TO")
            if not destination:
                raise VoiceyError(
                    "VY-DEP-004",
                    detail="a phone smoke requires --to E164 or VOICEY_SMOKE_TO.",
                )
            _confirm(
                (
        ...
        next_command = "docker compose -f compose.voicey.yaml up -d --build"
        if json_output:
            _json(
                {
                    "target": "docker",
        ...
        if manifest.phone_number is not None:
            console.print(
                ...
                f"voicey numbers point {manifest.phone_number} --url "
            ...
            if smoke_url is None and not skip_smoke:
                ...
                console.print(
                    "Then: voicey deploy docker --smoke https://voice.example.com "
...
def deploy_fly_command(
    ...
    rollback_created: Annotated[
        ...
        typer.Option(
            "--rollback-created",
            help="Permanently destroy only resources ledgered as created by voicey.",
        ),
    ] = False,
    skip_smoke: Annotated[
        bool,
    ...
    async def operation() -> None:
        ...
        if rollback_created:
            _confirm(
                (
                    "Permanently destroy only Fly resources marked created-by-voicey "
            ...
            next_command = (
                f"voicey deploy fly --app {app_name} --org {organization} "
            ...
            if json_output:
                ...
                console.print("Fly resources created by voicey were rolled back.")
                console.print(f"Next: {next_command}")
            return
        ...
        next_command = f"voicey deploy {next_target} --relay-url {plan.public_base} --yes"
        artifact_rows = {
...
def deploy_railway_command(
    ...
    rollback_created: Annotated[
        ...
        typer.Option(
            "--rollback-created",
            help="Delete only Railway resources ledgered as created by voicey.",
        ),
    ] = False,
    skip_smoke: Annotated[
        bool,
    ...
    async def operation() -> None:
        ...
        manifest_store = ManifestStore(context.root / "voicey.jsonc")
        base_command = (
            f"voicey deploy railway --project {shlex.quote(project_name)} "
        ...
        if rollback_created:
            _confirm(
                (
                    "Delete only Railway resources marked created-by-voicey for "
            ...
            if json_output:
                ...
                console.print("Railway resources created by voicey were rolled back.")
                console.print(f"Next: {payload['next_step']}")
            return
        ...
        next_target = "pipecat-cloud" if manifest.runtime == "pipecat" else "livekit-cloud"
        next_command = f"voicey deploy {next_target} --relay-url {report.state.public_base} --yes"
        artifact_rows = {
            "dockerfile": str(report.artifacts.dockerfile),
            "config": str(report.artifacts.config),
...
def deploy_pipecat_cloud_command(
    ...
    rollback_created: Annotated[
        ...
        typer.Option(
            "--rollback-created",
            help="Restore cutover and delete only a voicey-created cloud agent.",
        ),
    ] = False,
    engine_wheel: Annotated[
        Path | None,
    ...
    async def operation() -> None:
        ...
        if smoke_to is not None and skip_smoke:
            raise VoiceyError("VY-CLI-010", detail="--smoke-to conflicts with --skip-smoke.")
        ...
        if rollback_created:
            _confirm(
                (
                    ...
                    f"voicey-created Pipecat Cloud agent {agent_name}?"
                ),
                yes=yes,
            )
            ...
            payload = {
                ...
                "rolled_back": True,
                "resources": asdict(state),
                "next_step": "voicey deploy pipecat-cloud --prepare-only ...",
            }
            if json_output:
                _json(payload)
            else:
                console.print("Pipecat Cloud resources created by voicey were rolled back.")
                console.print(f"Next: {payload['next_step']}")
            return
    ...
    async def operation() -> None:
        ...
        if rollback:
            ...
            payload = {
                ...
                "rolled_back": True,
                "resources": asdict(state),
                "next_step": "voicey deploy livekit-cloud --help",
            }
            if json_output:
                _json(payload)
            else:
...
def upgrade_command(
    ...
    def operation() -> None:
        ...
        _confirm(
            (
                f"Resolve voicey from {channel}, replace uv.lock, and sync "
        ...
        if json_output:
            ...
            return
        console.print(
            f"voicey {report.from_version} -> {report.to_version} "
    ...
    *,
    ...
    if manifest.carriers == ["telnyx"]:
        return TelnyxAdapter(
            ...
            ledger_path=context.root / ".voicey" / "telephony.sqlite3",
        )
...
async def _verify_cloud_phone_smoke(
    ...
    *,
    ...
    relay = RelayCredential.parse(environment.get("VOICEY_RELAY_CREDENTIAL", ""))
...
def _provider_entry(context: ProjectContext, provider: str) -> ProviderCatalogEntry:
    ...
    if not matching:
        raise VoiceyError("VY-CLI-005", detail=f"unknown provider {provider!r}.")
    return matching[0]
...
def _confirm(message: str, *, yes: bool) -> None:
    ...
    if answer != "yes":
        raise VoiceyError("VY-CLI-008", detail="operation cancelled; no change applied.")
...
def _guard(
    ...
    *,
    ...
    try:
        ...
        issue = "https://github.com/voicey/voicey/issues/new?title=" + quote(
...
def _fail(error: VoiceyError, *, json_output: bool = False) -> NoReturn:
