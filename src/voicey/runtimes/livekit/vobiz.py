...
from voicey.telephony.models import validate_e164, validate_identifier
...
class VobizLiveKitSipProvisioner:
    ...
    async def provision(
        ...
        try:
            ...
            binding = ManagedSipResource(
                "vobiz_number_binding",
                inbound.resource.resource_id,
                _number_route(snapshot)
                != {
                    "application_id": None,
                    "trunk_group_id": inbound.resource.resource_id,
                },
            )
            operation = self._record(operation, binding)
            if binding.created:
                self.vobiz.attach_number(
        ...
        try:
            for wire in reversed(operation.resources):
                ...
                if resource.kind == "livekit_dispatch_rule":
                    ...
                    self.vobiz.restore_number(
                        operation.snapshot,
                        applied_trunk_id=resource.resource_id,
                    )
                else:
...
class VobizSipHTTPBackend:
        ...
        *,
        ...
        matches = [
            ...
            for item in self._list(
                f"{self.account_path}/trunks/credentials",
                "credentials",
            )
    ...
    def ensure_trunk(
        ...
        *,
        ...
        if direction not in {"inbound", "outbound"}:
            raise VoiceyError("VY-TEL-002", detail="Vobiz trunk direction is invalid.")
        matches = [
            item
        ...
        if len(matches) > 1:
            raise VoiceyError("VY-TEL-006", detail="duplicate managed Vobiz trunks.")
        desired = {
    ...
    def attach_number(
        ...
        *,
        ...
        snapshot = _number_route(expected_snapshot)
        current = _number_route(self._number(validated))
        applied = {"application_id": None, "trunk_group_id": desired}
        if current != snapshot:
            raise VoiceyError(
                "VY-TEL-006",
                detail="Vobiz number route changed before SIP assignment.",
            )
        if current == applied:
            return ManagedSipResource("vobiz_number_binding", desired, False)
        if current["trunk_group_id"] is not None:
            self._request(
                "DELETE",
        ...
        if current["application_id"] is not None:
            self._request(
                "DELETE",
                f"{self.account_path}/numbers/{quote(validated, safe='')}/application",
                expected=(200, 204),
        ...
        if _number_route(self._number(validated)) != applied:
            raise VoiceyError(
                "VY-TEL-011",
                detail="Vobiz did not confirm the number-to-trunk binding.",
            )
        return ManagedSipResource("vobiz_number_binding", desired, True)
    ...
    def restore_number(
        ...
        *,
        ...
        expected_applied = validate_identifier(
            applied_trunk_id,
            field_name="Vobiz applied trunk id",
        )
        previous = _number_route(snapshot)
        current = _number_route(self._number(number))
        if current == previous:
            return
        if current not in (
            {"application_id": None, "trunk_group_id": expected_applied},
            {"application_id": None, "trunk_group_id": None},
        ):
            raise VoiceyError(
                "VY-TEL-006",
                detail="Vobiz number route changed after SIP assignment.",
            )
        base = f"{self.account_path}/numbers/{quote(number, safe='')}"
        if current["trunk_group_id"] is not None:
            self._request(
                "DELETE",
                f"{base}/assign",
        ...
        if previous["trunk_group_id"] is not None:
            self._request(
                "POST",
                f"{base}/assign",
                json_body={"trunk_group_id": previous["trunk_group_id"]},
                expected=(204,),
                operation="restore number trunk",
            )
        elif previous["application_id"] is not None:
            self._request(
                "POST",
                f"{base}/application",
                json_body={"application_id": previous["application_id"]},
                expected=(204,),
                operation="restore number application",
            )
        if _number_route(self._number(number)) != previous:
            raise VoiceyError(
                "VY-TEL-006",
    ...
    def _list(self, path: str, key: str) -> list[dict[str, object]]:
        ...
        if isinstance(value, list):
            ...
            candidate = document.get(
                ...
                document.get(
                    "objects",
                    document.get("items", document.get("data", [])),
                ),
            )
            if candidate is None and any(
                envelope_key in document for envelope_key in (key, "objects", "items", "data")
            ):
                raw = []
            elif not isinstance(candidate, list):
                raise VoiceyError("VY-TEL-011", detail=f"Vobiz {key} list is malformed.")
            else:
                raw = cast("list[object]", candidate)
        else:
            raise VoiceyError("VY-TEL-011", detail=f"Vobiz {key} list is malformed.")
        if not all(isinstance(item, dict) for item in raw):
            raise VoiceyError("VY-TEL-011", detail=f"Vobiz {key} item is malformed.")
    ...
    def _request(
        ...
        *,
        ...
        if response.status_code not in expected:
            code = "VY-TEL-011" if response.status_code >= 500 else "VY-TEL-006"
            raise VoiceyError(
                code,
    ...
    def _object(value: object) -> dict[str, object]:
        if not isinstance(value, dict):
            raise VoiceyError("VY-TEL-011", detail="Vobiz response object is malformed.")
...
def _number_route(value: Mapping[str, object]) -> dict[str, str | None]:
    route = {
        "application_id": _optional(value.get("application_id")),
        "trunk_group_id": _optional(value.get("trunk_group_id")),
    }
    if all(route.values()):
        raise VoiceyError(
            "VY-TEL-006",
            detail="Vobiz number cannot be assigned to an application and trunk together.",
        )
    return route
...
def _sip_domain(value: object) -> str:
    ...
    if not _SIP_DOMAIN.fullmatch(domain):
        raise VoiceyError("VY-TEL-006", detail="Vobiz returned an invalid SIP domain.")
    return domain
