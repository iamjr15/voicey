...
class PlivoSipHTTPBackend:
    ...
    def ensure_uri(self, *, name: str, uri: str) -> ManagedSipResource:
        ...
        if len(matches) > 1:
            raise VoiceyError("VY-TEL-006", detail="duplicate managed Plivo SIP URIs.")
        if matches:
            item = matches[0]
    ...
    def ensure_credential(
        ...
        *,
        ...
        if len(matches) > 1:
            raise VoiceyError("VY-TEL-006", detail="duplicate managed Plivo credentials.")
        if matches:
            item = matches[0]
    ...
    def ensure_trunk(
        ...
        *,
        ...
        if direction not in {"inbound", "outbound"}:
            raise VoiceyError("VY-TEL-002", detail="Plivo trunk direction is invalid.")
        matches = [item for item in self._list("Trunk") if str(item.get("name", "")) == name]
        if len(matches) > 1:
            raise VoiceyError("VY-TEL-006", detail="duplicate managed Plivo trunks.")
        desired = {
...
def _sip_domain(value: object) -> str:
    ...
    if not _SIP_DOMAIN.fullmatch(domain):
        raise VoiceyError("VY-TEL-006", detail="Plivo returned an invalid SIP domain.")
    return domain
