...
from voicey.obs.records import Channel, Direction, NewCall, TimelineEvent
...
from voicey.runtimes.livekit.providers import DefaultLiveKitProviderFactory
...
from voicey.storage.models import ResultDeliveryConfig, ResultSnapshot, TerminalRequest
...
class LiveKitHostSettings:
    ...
    def __post_init__(self) -> None:
        if self.num_idle_processes < 0:
            raise VoiceyError("VY-RUN-002", detail="num_idle_processes cannot be negative.")
        if (
    ...
    async def reserve(self, call_id: str) -> None:
        ...
        async with self._lock:
            ...
            if call_id in self._pending or call_id in self._active:
                raise VoiceyError("VY-RUN-005", detail=f"duplicate call id {call_id!r}.")
            if self.occupied >= self.capacity:
                raise VoiceyError("VY-RUN-004", detail="LiveKit dispatch capacity is full.")
...
class LiveKitHost:
    ...
    def __init__(
        ...
        *,
        ...
        if agent.runtime != "livekit":
            raise VoiceyError("VY-RUN-001", detail="LiveKitHost requires runtime='livekit'.")
    ...
    async def on_request(self, request: JobRequest) -> None:
        ...
        if not await self.gate.admit(request.id, call_id):
            self.telemetry.record_error("VY-RUN-004")
            await request.reject(terminate=True)
            return
        try:
            await request.accept(
                identity=f"voicey-{self.agent.name}",
            ...
            self.telemetry.record_error("VY-RUN-006")
            raise
    ...
    async def reserve_web_call(self) -> str:
        """Persist a browser call before any Voicey or LiveKit token is exposed."""
...
def _prewarm_process(process: JobProcess) -> None:
    process.userdata["voicey_vad"] = silero.VAD.load()
...
def _metadata(raw: str) -> dict[str, str]:
    ...
    try:
        ...
        raise VoiceyError("VY-RUN-007", detail="LiveKit job metadata is invalid JSON.") from exc
