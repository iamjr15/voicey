...
from voicey.compatibility import warn_runtime_compatibility
...
class PipecatHost:
    ...
    def __init__(
        ...
        *,
        ...
        if agent.runtime != "pipecat":
            raise VoiceyError("VY-RUN-001", detail="PipecatHost requires runtime='pipecat'.")
        ...
        if agent.phone is not None and agent.phone.provider == "twilio" and twilio is None:
            raise VoiceyError(
                "VY-RUN-001",
                detail="Twilio phone config requires the voicey[twilio] adapter.",
            )
        if agent.phone is not None and agent.phone.provider == "telnyx" and telnyx is None:
            raise VoiceyError(
                "VY-RUN-001",
                detail="Telnyx phone config requires the voicey[telnyx] adapter.",
            )
        if agent.phone is not None and agent.phone.provider == "vobiz" and vobiz is None:
            raise VoiceyError(
                "VY-RUN-001",
                detail="Vobiz phone config requires the voicey[vobiz] adapter.",
            )
        if agent.phone is not None and agent.phone.provider == "plivo" and plivo is None:
            raise VoiceyError(
                "VY-RUN-001",
                detail="Plivo phone config requires the voicey[plivo] adapter.",
            )
        if (
            agent.web.enabled
    ...
    async def reserve_call(self, call: PipecatCall) -> _PendingCall:
        ...
        async with self._state_lock:
            ...
            pending.expires = asyncio.create_task(
                ...
                name=f"voicey-pending-{call.call_id}",
            )
    ...
    async def drain(self, *, timeout_s: float | None = None) -> DrainReport:
        ...
        if timeout_s is not None and timeout_s <= 0:
            raise VoiceyError("VY-RUN-008", detail="drain timeout must be positive.")
    ...
    def _build_app(self) -> FastAPI:
        ...
        app = FastAPI(title=f"voicey:{self.agent.name}", lifespan=lifespan)

        @app.exception_handler(VoiceyError)
        async def voicey_error_handler(  # pyright: ignore[reportUnusedFunction]
            _request: Request,
            error: VoiceyError,
        ) -> JSONResponse:
        ...
        async def recording_artifact(  # pyright: ignore[reportUnusedFunction]
            ...
            if self.recording_handler is None:
                return _error_response("VY-RES-010", 404)
            try:
                ...
                return _error_response(
                    exc.code,
                    403 if exc.code == "VY-WEB-004" else 404,
                )
            return Response(
                content=content,
        ...
        async def twilio_answer(  # pyright: ignore[reportUnusedFunction]
            ...
            if not adapter.verify_request(telephony):
                return _error_response("VY-RUN-007", 403)
            ...
            try:
                ...
                if exc.code not in {"VY-RUN-004", "VY-RUN-008"}:
                    raise
                return Response(
            ...
            target = replace(
                self.twilio_target,
                custom_parameters={
                    "voicey_token": pending.admission.token,
        ...
        async def twilio_media(  # pyright: ignore[reportUnusedFunction]
            ...
            if not adapter.verify_request(_websocket_telephony_request(websocket)):
                await websocket.close(code=1008, reason="VY-RUN-007")
                return
            ...
            try:
                ...
                with suppress(RuntimeError):
                    await websocket.close(code=1011, reason="VY-RUN-006")
            finally:
        ...
        async def twilio_recordings(  # pyright: ignore[reportUnusedFunction]
            ...
            if event.type not in {"recording_ready", "recording_failed"}:
                return _error_response("VY-TEL-009", 400)
            if self.recording_handler is None:
                return _error_response("VY-TEL-009", 503)
            try:
        ...
        async def twilio_amd(  # pyright: ignore[reportUnusedFunction]
            ...
            if event.answered_by is None:
                return _error_response("VY-RUN-007", 400)
        ...
        async def web_offer(  # pyright: ignore[reportUnusedFunction]
            ...
            if offer.pc_id is not None and offer.pc_id in self._web_sessions:
                ...
                if answer is None:
                    raise VoiceyError("VY-RUN-007", detail="WebRTC renegotiation had no answer.")
                return answer
            ...
            try:
                    ...
                    try:
                        ...
                        self._track_session(
                            asyncio.create_task(
                                ...
                                name=f"voicey-web-{call.call_id}",
                            )
                        )
                ...
                if answer is None:
                    raise VoiceyError("VY-RUN-007", detail="WebRTC offer produced no answer.")
                return answer
            except Exception:
    ...
    async def _authorize_web(self, request: Request, *, pc_id: str | None) -> object | None:
        if self.web_sessions is None:
            if self.settings.allow_insecure_web_sessions_for_tests:
                return None
            raise VoiceyError("VY-WEB-001", detail="browser session security is unavailable.")
    ...
    def _require_twilio(self) -> TwilioRuntimeAdapter:
        if self.twilio is None:
            raise VoiceyError("VY-RUN-001", detail="Twilio adapter is not configured.")
        return self.twilio

    def _require_telnyx(self) -> TelnyxRuntimeAdapter:
        if self.telnyx is None:
            raise VoiceyError("VY-RUN-001", detail="Telnyx adapter is not configured.")
        return self.telnyx

    def _require_vobiz(self) -> VobizRuntimeAdapter:
        if self.vobiz is None:
            raise VoiceyError("VY-RUN-001", detail="Vobiz adapter is not configured.")
        return self.vobiz

    def _require_plivo(self) -> PlivoRuntimeAdapter:
        if self.plivo is None:
            raise VoiceyError("VY-RUN-001", detail="Plivo adapter is not configured.")
        return self.plivo
...
def _twilio_handshake(call_data: CallData) -> tuple[str, str, str]:
    ...
    token = str(body.get("voicey_token", ""))
...
def _vobiz_handshake(raw_frame: str) -> tuple[str, str]:
    ...
    try:
        ...
        raise VoiceyError("VY-RUN-005", detail="Vobiz start frame is invalid JSON.") from exc
    if not isinstance(payload_value, dict):
        raise VoiceyError("VY-RUN-005", detail="Vobiz start frame must be an object.")
    payload = cast("dict[str, object]", payload_value)
    if payload.get("event") != "start":
        raise VoiceyError("VY-RUN-005", detail="Vobiz media must begin with a start frame.")
    start_value = payload.get("start")
    if not isinstance(start_value, dict):
        raise VoiceyError("VY-RUN-005", detail="Vobiz start frame lacks start data.")
    ...
    if not isinstance(media_value, dict):
        raise VoiceyError("VY-RUN-005", detail="Vobiz start frame lacks media format.")
    ...
    if not isinstance(sample_rate_value, (str, int)) or isinstance(sample_rate_value, bool):
        raise VoiceyError("VY-RUN-005", detail="Vobiz sample rate is invalid.")
    try:
        sample_rate = int(sample_rate_value)
    except (TypeError, ValueError) as exc:
        raise VoiceyError("VY-RUN-005", detail="Vobiz sample rate is invalid.") from exc
    ...
    if not call_id or not stream_id:
        raise VoiceyError("VY-RUN-005", detail="Vobiz start frame lacks call or stream data.")
...
def _plivo_handshake(raw_frame: str) -> tuple[str, str]:
    ...
    try:
        ...
        raise VoiceyError("VY-RUN-005", detail="Plivo start frame is invalid JSON.") from exc
    if not isinstance(payload_value, dict):
        raise VoiceyError("VY-RUN-005", detail="Plivo start frame must be an object.")
    payload = cast("dict[str, object]", payload_value)
    if payload.get("event") != "start":
        raise VoiceyError("VY-RUN-005", detail="Plivo media must begin with a start frame.")
    start_value = payload.get("start")
    if not isinstance(start_value, dict):
        raise VoiceyError("VY-RUN-005", detail="Plivo start frame lacks start data.")
    start = cast("dict[str, object]", start_value)
    call_id = str(start.get("callId", ""))
    stream_id = str(start.get("streamId", payload.get("streamId", "")))
    media_value = start.get("mediaFormat")
    if not isinstance(media_value, dict):
        raise VoiceyError("VY-RUN-005", detail="Plivo start frame lacks media format.")
    media = cast("dict[str, object]", media_value)
    content_type = str(media.get("contentType", media.get("encoding", ""))).casefold()
    sample_rate_value = media.get("sampleRate", 0)
    if not isinstance(sample_rate_value, (str, int)) or isinstance(sample_rate_value, bool):
        raise VoiceyError("VY-RUN-005", detail="Plivo sample rate is invalid.")
    try:
        sample_rate = int(sample_rate_value)
    except (TypeError, ValueError) as exc:
        raise VoiceyError("VY-RUN-005", detail="Plivo sample rate is invalid.") from exc
    if content_type not in {"audio/x-mulaw", "pcmu", "mulaw"} or sample_rate != 8000:
        raise VoiceyError(
            "VY-RUN-005",
    ...
    if not call_id or not stream_id:
        raise VoiceyError("VY-RUN-005", detail="Plivo start frame lacks call or stream data.")
    return call_id, stream_id
