...
class RecipeDriftAnalyzer:
    ...
    def analyze(self, manifest: ProjectManifest) -> RecipeDriftReport:
        ...
        next_step = (
            "review the AI merge guidance, apply the recipe update, then run voicey test"
            if status == "update-available"
            else "review voicey.recipe-lock.json and restore the missing baseline"
            if status == "baseline-missing"
            else "review the AI merge guidance, then run voicey test"
            if conflicts
            else "voicey doctor"
        )
        return RecipeDriftReport(
    ...
    def _baseline(
        ...
        if (
            ...
            raise VoiceyError(
                "VY-UPG-003",
                detail=(
                    "voicey.recipe-lock.json does not match the manifest recipe, "
                    "version, or runtime."
                ),
            )
    ...
    def _merge_prompt(
        ...
        return (
            "Merge this voicey recipe update without overwriting project code.\n"
            f"Recipe: {manifest.recipe.name} {manifest.recipe.version} -> "
            f"{upstream_version}; runtime: {manifest.runtime}.\n"
            "The base sources are in voicey.recipe-lock.json; local sources are "
            "the project files; upstream sources are the installed voicey "
            ...
            "Merge each hunk, run the recipe scenario suite and `voicey test`, then "
...
def _semver_key(value: str) -> tuple[int, int, int, int, tuple[str, ...]]:
    ...
    if match is None:
        raise VoiceyError("VY-UPG-003", detail=f"recipe version {value!r} is not SemVer.")
