...
from voicey.testing.models import JudgeConfig, ScenarioDefinition, ScenarioTurn
...
class QueueAudioInput(AudioInput):
    ...
    def __init__(self) -> None:
        super().__init__(label="voicey-sim-caller")
            ...
            for turn in turns:
                ...
                if expectation.judge:
                    decision = await TranscriptJudge(
                        build_model_client(judge, environment=environment)
                    ).evaluate(
                        expectation.judge,
                        tuple(transcript),
