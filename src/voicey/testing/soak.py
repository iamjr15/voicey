...
from voicey.config.models import Agent, Models, Results, Web
from voicey.errors import VoiceyError
from voicey.obs.records import TimelineEvent, TranscriptTurn
...
class SoakReport:
    ...
    def assert_healthy(self) -> None:
        if self.failures:
            raise VoiceyError("VY-TST-005", detail="; ".join(self.failures))
...
async def run_engine_soak(
    ...
    *,
    ...
    if not runtimes or len(runtimes) != len(set(runtimes)):
        raise VoiceyError("VY-TST-005", detail="soak runtimes must be unique and non-empty.")
...
async def _one_soak_call(
    ...
    *,
    ...
    if observed.event_id != terminal.event_id:
        raise VoiceyError("VY-TST-005", detail=f"{call_id} terminal event changed.")
