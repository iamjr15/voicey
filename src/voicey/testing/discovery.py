...
from voicey.testing.models import ScenarioDefinition

ScenarioFunction = Callable[[], Mapping[str, Any] | ScenarioDefinition]
F = TypeVar("F", bound=ScenarioFunction)
_SCENARIO_MARKER = "__voicey_scenario__"


def scenario(function: F) -> F:
    """Mark a zero-argument owned function as a voicey scenario."""
...
def discover_scenarios(root: Path) -> tuple[ScenarioDefinition, ...]:
    ...
    with _project_path(root):
        for index, path in enumerate(files):
            module_name = f"_voicey_scenarios_{index}_{path.stem}"
...
def _load_module(path: Path, module_name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise VoiceyError("VY-TST-001", detail=f"cannot import {path}.")
