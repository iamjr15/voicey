...
class JudgeConfig(VoiceyModel):
    """Local or cloud test-model endpoint; local Ollama is the explicit default."""

    service: Literal["ollama", "openai", "anthropic"] = "ollama"
    model: str = "qwen3:8b"
    ...
    def cloud_judge_has_key_reference(self) -> JudgeConfig:
        if self.service in {"openai", "anthropic"} and not self.api_key_env:
...
class LiveTestingConfig(VoiceyModel):
    ...
    public_url_env: str = "VOICEY_LIVE_PUBLIC_URL"
    target_number_env: str = "VOICEY_LIVE_TARGET_NUMBER"
    twilio_from_number_env: str = "VOICEY_LIVE_TWILIO_FROM"
    livekit_outbound_trunk_env: str = "VOICEY_LIVEKIT_OUTBOUND_TRUNK_ID"
    paid_ack_env: str = "VOICEY_LIVE_PSTN_ACK"
    max_calls_env: str = "VOICEY_LIVE_PSTN_MAX_CALLS"
...
class TestingConfig(VoiceyModel):
...
class ToolExpectation(VoiceyModel):
    ...
    model_config = VoiceyModel.model_config | {"arbitrary_types_allowed": True}
...
class TurnExpectation(VoiceyModel):
...
class ScenarioTurn(VoiceyModel):
    ...
    def turn_has_input_or_expectation(self) -> ScenarioTurn:
        ...
        if not self.runtimes:
            raise ValueError("turn must target at least one runtime")
        return self


class ResultExpectation(VoiceyModel):
...
class ScenarioMetrics(VoiceyModel):
...
class ScenarioDefinition(VoiceyModel):
