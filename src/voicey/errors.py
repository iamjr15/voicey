"""Stable application errors backed by the public voicey error catalog."""

from __future__ import annotations

from dataclasses import dataclass

ERROR_DOCS_BASE = "https://github.com/voicey/voicey/blob/main/docs/errors.md"
...
ERROR_CATALOG: dict[str, ErrorDefinition] = {
    ...
    "VY-TEL-001": ErrorDefinition(
        code="VY-TEL-001",
        cause="The requested telephony adapter or its optional dependency is unavailable.",
        fix='Install the exact carrier extra, for example `uv pip install "voicey[twilio]"`.',
    ),
    "VY-TEL-002": ErrorDefinition(
        code="VY-TEL-002",
        cause="A telephony target, credential, number, or adapter setting is invalid.",
        fix="Correct the reported carrier setting and run `voicey doctor` before retrying.",
    ),
    "VY-TEL-003": ErrorDefinition(
        code="VY-TEL-003",
    ...
    "VY-RUN-001": ErrorDefinition(
        code="VY-RUN-001",
        cause="The selected runtime or one of its pinned optional dependencies is unavailable.",
        fix="Install the runtime extra printed by the command and rerun `voicey doctor`.",
    ),
    "VY-RUN-002": ErrorDefinition(
        code="VY-RUN-002",
        cause="A runtime provider model, credential, or voice setting cannot be constructed.",
        fix="Correct the reported provider setting or key, then rerun `voicey doctor`.",
    ),
    "VY-RUN-003": ErrorDefinition(
        code="VY-RUN-003",
    ...
    "VY-DEP-001": ErrorDefinition(
        code="VY-DEP-001",
        cause="A deployment artifact conflicts with an existing project file.",
        fix="Review or move the reported file; voicey never overwrites deployment edits.",
    ),
    "VY-DEP-002": ErrorDefinition(
        code="VY-DEP-002",
    ...
    "VY-DEP-003": ErrorDefinition(
        code="VY-DEP-003",
        cause="The production container runtime configuration is incomplete or invalid.",
        fix="Set every documented deployment environment variable, then rerun `voicey doctor`.",
    ),
    ...
    "VY-DEP-009": ErrorDefinition(
        ...
        fix=(
            ...
            "then rerun the same resumable voicey command."
        ),
    ),
    "VY-DEP-010": ErrorDefinition(
        code="VY-DEP-010",
    ...
    "VY-TST-001": ErrorDefinition(
        ...
        fix="Correct the reported tests/scenarios source or tests/voicey-test.jsonc field.",
    ),
    "VY-TST-002": ErrorDefinition(
        code="VY-TST-002",
    ...
    "VY-CLI-002": ErrorDefinition(
        code="VY-CLI-002",
        cause="The guided setup was cancelled, conflicted, or cannot be resumed safely.",
        fix="Rerun `voicey init --resume` or choose a new empty project directory.",
    ),
    "VY-CLI-003": ErrorDefinition(
        code="VY-CLI-003",
        cause="A project scaffold or protected environment file could not be written safely.",
        fix="Check project permissions and disk space, then resume `voicey init`.",
    ),
    "VY-CLI-004": ErrorDefinition(
        code="VY-CLI-004",
    ...
    "VY-CLI-006": ErrorDefinition(
        code="VY-CLI-006",
        cause="One or more doctor preflight checks failed.",
        fix="Apply each printed advice line, then rerun `voicey doctor`.",
    ),
    "VY-CLI-007": ErrorDefinition(
        code="VY-CLI-007",
    ...
    "VY-UPG-001": ErrorDefinition(
        code="VY-UPG-001",
        cause="The project or installed uv version cannot support a safe voicey upgrade.",
        fix="Use a regular uv-managed project with a direct voicey dependency and uv >=0.11,<1.",
    ),
    "VY-UPG-002": ErrorDefinition(
        code="VY-UPG-002",
        cause="The lockfile-only voicey upgrade or fresh-process verification failed.",
        fix="Review the safe error detail; the prior lockfile was restored when available.",
    ),
    "VY-UPG-003": ErrorDefinition(
        code="VY-UPG-003",
...
class VoiceyError(RuntimeError):
    ...
    def __init__(self, code: str, *, detail: str | None = None) -> None:
        try:
            ...
            msg = f"unregistered voicey error code: {code}"
