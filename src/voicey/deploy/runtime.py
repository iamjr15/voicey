...
from voicey.obs.telemetry import InstrumentedRepository
...
from voicey.runtimes.pipecat import (
    PipecatHost,
    PipecatHostSettings,
...
class ContainerSettings:
    ...
    def from_environment(
        ...
        *,
        ...
        public_base = environment.get("VOICEY_PUBLIC_BASE", "").rstrip("/")
        ...
        if (
            ...
            raise VoiceyError(
                "VY-DEP-003",
                detail="VOICEY_PUBLIC_BASE must be an HTTPS base URL.",
            )
        data_dir = Path(environment.get("VOICEY_DATA_DIR", "/app/data"))
        if not data_dir.is_absolute():
            raise VoiceyError("VY-DEP-003", detail="VOICEY_DATA_DIR must be absolute.")
        public_port = _integer(environment, "VOICEY_PORT", default=7860)
        admin_port = _integer(environment, "VOICEY_ADMIN_PORT", default=7861)
        if not 1 <= public_port <= 65535 or not 1 <= admin_port <= 65535:
            raise VoiceyError("VY-DEP-003", detail="container ports must be in 1-65535.")
        admin_origin = environment.get("VOICEY_ADMIN_ORIGIN", "http://agent:7861")
        if not _is_origin(admin_origin):
            raise VoiceyError(
                "VY-DEP-003",
                detail="VOICEY_ADMIN_ORIGIN must be a normalized HTTP(S) origin.",
            )
        trusted_proxy_ips = _trusted_ips(environment.get("VOICEY_TRUSTED_PROXY_IPS", ""))
        trusted_proxy_cidrs = _trusted_cidrs(environment.get("VOICEY_TRUSTED_PROXY_CIDRS", ""))
        return cls(
            ...
            deploy_target=environment.get("VOICEY_DEPLOY_TARGET", ""),
            storage_backend=environment.get("VOICEY_STORAGE_BACKEND", ""),
            sqlite_local_only=_flag(environment, "VOICEY_SQLITE_LOCAL_ONLY"),
            replica_count=_integer(environment, "VOICEY_REPLICA_COUNT", default=0),
            trusted_proxy_ips=trusted_proxy_ips,
            trusted_proxy_cidrs=trusted_proxy_cidrs,
            integrator_secret=environment.get("VOICEY_INTEGRATOR_SECRET") or None,
        )
...
async def run_container(
    *,
    ...
    if not rolling.stale_writer_rejected or rolling.terminal_event_count != 1:
        raise VoiceyError("VY-DEP-002", detail="rolling-generation invariant did not hold.")
    ...
    async with SQLiteRepository(database_path) as repository:
        ...
        if agent.web.enabled:
            if settings.integrator_secret is None:
                raise VoiceyError(
                    "VY-DEP-003",
                    detail="web deployments require VOICEY_INTEGRATOR_SECRET.",
                )
...
async def _supervise(
    *,
    ...
    server_tasks = [
        asyncio.create_task(server.serve(), name=f"voicey-listener-{server.config.port}")
        for server in servers
    ]
    delivery_stop = asyncio.Event()
    delivery_task = asyncio.create_task(
        _delivery_loop(delivery, delivery_stop),
        name="voicey-result-delivery",
    )
    shutdown = asyncio.Event()
    restore_signals = _install_signal_handlers(shutdown)
    shutdown_task = asyncio.create_task(shutdown.wait(), name="voicey-signal-wait")
    try:
        ...
        if unexpected:
            await unexpected[0]
            raise VoiceyError("VY-DEP-003", detail="a container service exited early.")
    finally:
    ...
    *,
    ...
    async def wait() -> None:
        while not server.started:
            if task.done():
                await task
                raise VoiceyError("VY-DEP-003", detail="container listener exited early.")
...
def _load_agent(module_name: str) -> Agent:
    ...
    if not isinstance(agent, Agent):
        raise VoiceyError(
            "VY-DEP-003",
            detail=f"{module_name}.agent is not a voicey Agent.",
        )
    return agent
...
def _required_secret(environment: Mapping[str, str], name: str) -> str:
    ...
    if not value:
        raise VoiceyError("VY-DEP-003", detail=f"required secret variable {name} is empty.")
    return value
...
def _trusted_ips(value: str) -> frozenset[str]:
    ...
    try:
        ...
        raise VoiceyError(
            "VY-DEP-003",
            detail="VOICEY_TRUSTED_PROXY_IPS contains an invalid address.",
        ) from exc
    return values
...
def _trusted_cidrs(value: str) -> tuple[str, ...]:
    ...
    try:
        ...
        raise VoiceyError(
            "VY-DEP-003",
            detail="VOICEY_TRUSTED_PROXY_CIDRS contains an invalid network.",
        ) from exc
    return values
...
def _origin(value: str) -> str:
    ...
    if not parsed.scheme or not parsed.netloc:
        raise VoiceyError("VY-DEP-003", detail="public base has no origin.")
