    ...
    def run(
        ...
        *,
        ...
        if check and result.returncode != 0:
            ...
            raise VoiceyError("VY-DEP-006", detail=detail)
        return result
...
class FlyResourceState:
    ...
    def from_payload(cls, payload: object) -> FlyResourceState:
        if not isinstance(payload, dict):
            raise VoiceyError("VY-DEP-007", detail="Fly resource ledger is not an object.")
        try:
...
class FlyArtifactGenerator:
    ...
    def __init__(self, project_root: Path) -> None:
        ...
        self.directory = self.project_root / ".voicey" / "deploy" / "fly"
    ...
    def generate(
        ...
        *,
        ...
        try:
            ...
            raise VoiceyError("VY-DEP-003", detail="generated Fly TOML is invalid.") from exc
        if (
            parsed.get("app") != plan.app_name
            or parsed.get("http_service", {}).get("internal_port") != 8080
            or parsed.get("env", {}).get("VOICEY_DEPLOY_TARGET") != "fly"
        ):
            raise VoiceyError(
                "VY-DEP-003",
...
class FlyDeploymentManager:
    ...
    def __init__(
        ...
        *,
        ...
        self.store = FlyResourceStore(
            self.project_root / ".voicey" / "deploy" / "fly-resources.json"
        )
    ...
    def rollback_created(self, plan: FlyPlan) -> FlyResourceState:
        """Destroy only resources explicitly recorded as created by voicey."""
        state = self.store.load()
        if state is None:
            raise VoiceyError("VY-DEP-007", detail="no Fly resource ledger exists.")
        state.validate_plan(plan)
    ...
    def _ensure_postgres(
        ...
        if len(matches) > 1:
            raise VoiceyError("VY-DEP-007", detail="managed Postgres identity is ambiguous.")
        if matches:
            ...
            if state.postgres_id not in {None, cluster_id}:
                raise VoiceyError("VY-DEP-007", detail="managed Postgres id drifted.")
...
def _dockerfile(wheel: Path | None) -> str:
    ...
    copy = f"COPY .voicey/deploy/fly/{wheel.name} /tmp/{wheel.name}\n" if wheel else ""
    return f"""# syntax=docker/dockerfile:1.7
# Generated by voicey {__version__}. Secrets are supplied by Fly at runtime.
...
def _fly_config(plan: FlyPlan) -> str:
    ...
    values = {
        "VOICEY_PUBLIC_BASE": plan.public_base,
        "VOICEY_OBJECT_BUCKET": plan.bucket_name,
        "VOICEY_OBJECT_PREFIX": "voicey",
        "VOICEY_OBJECT_ENDPOINT": "https://fly.storage.tigris.dev",
        "AWS_REGION": "auto",
    ...
    return f"""# Generated by voicey {__version__}. Do not add secrets here.
...
[build]
  dockerfile = ".voicey/deploy/fly/Dockerfile.results"
  ignorefile = ".voicey/deploy/fly/dockerignore"
...
def _dockerignore(wheel: Path | None) -> str:
    include = f"!.voicey/deploy/fly/{wheel.name}\n" if wheel is not None else ""
    return f"""*
!.voicey/
!.voicey/deploy/
!.voicey/deploy/fly/
