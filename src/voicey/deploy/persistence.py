...
from voicey.security.files import ensure_private_directory
...
async def docker_persistence_preflight(
    ...
    *,
    ...
    if (
        ...
        raise VoiceyError(
            ...
            detail=(
                ...
                "VOICEY_SQLITE_LOCAL_ONLY=1, and replica_count=1."
            ),
        )
    ...
    if observed != expected:
        raise VoiceyError("VY-DEP-002", detail="local artifact round trip changed bytes.")
...
async def rolling_generation_invariant(data_dir: Path) -> RollingGenerationReport:
    ...
    delivery = ResultDeliveryConfig(endpoint="https://example.invalid/voicey-results")
    try:
        async with (
...
def _prepare_data_root(data_dir: Path) -> Path:
    if data_dir.is_symlink():
        raise VoiceyError("VY-DEP-002", detail=f"{data_dir} must not be a symbolic link.")
