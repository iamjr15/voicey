...
class ConfigValidationError(VoiceyError):
    ...
    def __init__(self, issues: tuple[ConfigIssue, ...]) -> None:
        ...
        super().__init__("VY-CFG-001", detail=detail)
        ...
        if not environ.get(env_name):
            issues.append(
                ConfigIssue(
                    ...
                    fix=f"Run `voicey keys add {owner}` or inject {env_name} in CI.",
                )
            )
        ...
        try:
            ...
            issues.append(
                ConfigIssue(
                    ...
                    fix=f"Rotate it with `voicey keys add webhook --env {env_name}`.",
                )
            )
