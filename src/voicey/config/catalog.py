...
from voicey.config.models import RuntimeName, VoiceyModel
...
# Explicit IDs keep ``Voice(id=None)`` deterministic across runtime SDKs. The
# SDK defaults currently differ and Pipecat 1.6.0 serializes an omitted
# Cartesia voice as an invalid empty ID.
CURATED_DEFAULT_VOICE_IDS: dict[str, str] = {
    "cartesia/sonic-3.5": "db6b0ed5-d5d3-463d-ae85-518a07d3c2b4",
    "elevenlabs/flash-2.5": "hpp4J3VqNfWAUOO0d1Us",
    "openai/gpt-4o-mini-tts": "alloy",
}


def resolve_voice_id(model_id: str, configured_voice_id: str | None) -> str:
    """Resolve an explicit voice or the curated default for a catalog TTS model."""
    if configured_voice_id is not None:
        return configured_voice_id
    try:
        return CURATED_DEFAULT_VOICE_IDS[model_id]
    except KeyError as exc:
        msg = f"{model_id!r} has no curated default voice."
        raise ValueError(msg) from exc


class ProviderCatalogEntry(VoiceyModel):
    ...
    model_config = VoiceyModel.model_config | {"frozen": True}
...
def _entry(
    *,
    ...
    description: str,
    runtimes: frozenset[RuntimeName] = frozenset({"pipecat", "livekit"}),
...
DEFAULT_PROVIDER_CATALOG = ProviderCatalog(
    (
        ...
        _entry(
            ...
            url="https://api.plivo.com/v1/Account/${PLIVO_AUTH_ID}/",
            headers={"Authorization": "Basic ${PLIVO_AUTH_ID}:${PLIVO_AUTH_TOKEN}"},
            native_idempotency=False,
            description="Programmable voice and Zentrunk carrier; launch tier is beta.",
        ),
        _entry(
            id="sip",
            kind="carrier",
            ...
            url="https://cloud-api.livekit.io/api/sip",
            headers={},
            native_idempotency=False,
            description="Operator-managed direct SIP trunk; LiveKit-only beta escape hatch.",
