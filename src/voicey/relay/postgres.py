...
from voicey.storage.postgres import PostgresMigrator
...
class PostgresRelayJournal:
    ...
    def __init__(
        ...
        *,
        ...
        if not dsn or min_size < 0 or max_size < 1 or min_size > max_size or timeout_s <= 0:
            raise VoiceyError("VY-REL-006", detail="Postgres journal settings are invalid.")
    ...
    async def open(self) -> Self:
        ...
        try:
            ...
            await self._pool.close()
            raise VoiceyError("VY-REL-006", detail="Postgres relay pool is unavailable.") from exc
        self._open = True
        return self
    ...
    async def reserve_request(
        ...
        *,
        ...
        try:
            ...
            raise VoiceyError("VY-REL-006", detail="could not reserve relay request.") from exc
    ...
    async def next_sequence(self, call_id: str) -> int:
        try:
            ...
            raise VoiceyError("VY-REL-006", detail="could not read stream cursor.") from exc
    ...
    async def _execute_count(
        ...
        try:
            ...
            raise VoiceyError("VY-REL-006", detail="Postgres journal mutation failed.") from exc
