...
class RelayCredential:
    ...
    def parse(cls, value: str) -> RelayCredential:
        ...
        if prefix != "vkr" or not separator or not separator_two:
            raise VoiceyError("VY-REL-001", detail="relay credential format is invalid.")
        try:
...
class RelayRequestSigner:
    ...
    def headers(self, method: str, path: str, body: bytes) -> dict[str, str]:
        ...
        return {
            ...
            "x-voicey-relay-timestamp": timestamp,
            "x-voicey-relay-nonce": nonce,
            "x-voicey-relay-signature": signature,
...
class RelayRequestVerifier:
    ...
    def __init__(
        ...
        *,
        ...
        if tolerance <= timedelta(0) or tolerance > timedelta(hours=1):
            raise VoiceyError("VY-REL-001", detail="relay replay tolerance is invalid.")
    ...
    def verify(
        ...
        *,
        ...
        timestamp = headers.get("x-voicey-relay-timestamp", "")
        nonce = headers.get("x-voicey-relay-nonce", "")
        supplied = headers.get("x-voicey-relay-signature", "")
        credential = self._keyring.find(key_id) if scheme == _AUTH_SCHEME and separator else None
        try:
            seconds = int(timestamp)
        except ValueError as exc:
            raise VoiceyError("VY-REL-003", detail="relay timestamp is malformed.") from exc
        now = self._clock()
        if (
            credential is None
            or len(nonce) < 20
            ...
            raise VoiceyError("VY-REL-003", detail="relay authorization is invalid or expired.")
        ...
        if not hmac.compare_digest(supplied, expected):
            raise VoiceyError("VY-REL-003", detail="relay request signature does not match.")
...
class FenceSigner:
    ...
    def __init__(
        ...
        *,
        ...
        if token_ttl < timedelta(minutes=5) or token_ttl > timedelta(hours=24):
            raise VoiceyError("VY-REL-001", detail="relay fence TTL is invalid.")
        self._keyring = keyring
    ...
    def verify(self, token: str, *, call_id: str) -> CallLease:
        ...
        if credential is None or not separator or not separator_two:
            raise VoiceyError("VY-REL-004", detail="fence token format is invalid.")
        try:
            ...
            raise VoiceyError("VY-REL-004", detail="fence token payload is invalid.") from exc
        if (
            ...
            raise VoiceyError("VY-REL-004", detail="fence token is invalid or expired.")
        return CallLease(
