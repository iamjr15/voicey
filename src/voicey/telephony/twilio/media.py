...
class JitterBuffer:
    ...
    def __init__(self, *, max_late_frames: int = 3) -> None:
        if max_late_frames < 0:
            raise VoiceyError("VY-TEL-010", detail="jitter tolerance cannot be negative.")
    ...
    def push(self, frame: MediaFrame) -> tuple[MediaFrame, ...]:
        ...
        if frame.sequence < 0 or frame.timestamp_ms < 0 or len(frame.payload) != 160:
            raise VoiceyError("VY-TEL-010", detail="invalid Twilio 20ms media frame.")
    ...
    def __init__(
        ...
        *,
        ...
        if frame_duration_s <= 0:
            raise VoiceyError("VY-TEL-010", detail="frame duration must be positive.")
...
class PlaybackTracker:
    ...
    def sent(self, name: str) -> None:
        if not _MARK_NAME.fullmatch(name) or name in self._pending:
            raise VoiceyError("VY-TEL-010", detail="invalid or duplicate playback mark.")
    ...
    def acknowledged(self, name: str) -> float:
        try:
            ...
            raise VoiceyError(
                "VY-TEL-010", detail="unknown playback mark acknowledgement."
            ) from exc
    ...
    try:
        ...
        if event == "dtmf":
            ...
            if digit not in "0123456789*#":
                raise VoiceyError("VY-TEL-010", detail="invalid inbound DTMF digit.")
        ...
        raise VoiceyError("VY-TEL-010", detail="malformed Twilio media message.") from exc
    raise VoiceyError("VY-TEL-010", detail=f"unsupported Twilio media event {event!r}.")
...
def mulaw_to_linear16(value: int) -> int:
    ...
    if not 0 <= value <= 255:
        raise VoiceyError("VY-TEL-010", detail="μ-law byte is outside 0..255.")
    inverted = (~value) & 0xFF
...
def linear16_to_mulaw(sample: int) -> int:
    ...
    if not -32768 <= sample <= 32767:
        raise VoiceyError("VY-TEL-010", detail="PCM sample is outside signed 16-bit range.")
...
def resample_linear(
    ...
    *,
    ...
    if source_rate <= 0 or target_rate <= 0:
        raise VoiceyError("VY-TEL-010", detail="sample rates must be positive.")
    if not samples:
        return ()
...
def tone_pcm16(
    *,
    ...
    if (
        ...
        raise VoiceyError("VY-TEL-010", detail="invalid certification tone settings.")
...
def dominant_frequency(samples: Sequence[int], *, sample_rate: int) -> float:
    ...
    if len(samples) < 2 or sample_rate <= 0:
        raise VoiceyError("VY-TEL-010", detail="not enough PCM for frequency detection.")
...
def _validate_stream_sid(value: str) -> str:
    if not _STREAM_SID.fullmatch(value):
        raise VoiceyError("VY-TEL-010", detail="invalid Twilio StreamSid.")
    return value


def _validate_call_sid(value: str) -> str:
    if not _CALL_SID.fullmatch(value):
        raise VoiceyError("VY-TEL-010", detail="invalid Twilio CallSid.")
    return value


def _validate_mark(value: str) -> str:
    if not _MARK_NAME.fullmatch(value):
        raise VoiceyError("VY-TEL-010", detail="invalid Twilio playback mark.")
    return value
