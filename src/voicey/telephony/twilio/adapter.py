...
class TwilioAdapter:
    ...
    def buy_number(self, country: str, area: str | None = None) -> NumberInfo:
        ...
        if area is not None and not area.isdigit():
            raise VoiceyError("VY-TEL-002", detail="Twilio area code must contain only digits.")
        try:
    ...
    def release_number(self, number: str) -> None:
        ...
        if not deleted:
            raise VoiceyError("VY-TEL-004", detail="Twilio did not confirm number release.")
    ...
    def restore(self, token: RollbackToken) -> None:
        if token.provider != self.provider:
            raise VoiceyError("VY-TEL-006", detail="rollback token belongs to another provider.")
    ...
    async def download_recording(
        ...
        *,
        ...
        try:
            ...
            raise VoiceyError("VY-TEL-009", detail="invalid Twilio RecordingSid.") from exc
        if max_bytes <= 0:
...
def _checked_twiml(response: VoiceResponse) -> str:
    ...
    if len(value.encode()) > 4096:
        raise VoiceyError("VY-TEL-002", detail="inline TwiML exceeds the 4KB carrier limit.")
    return value
...
def _warm_url_from_base(base: str | None, transfer_id: str, action: str) -> str:
    ...
    if action not in {"accept", "events", "conference"}:
        raise VoiceyError("VY-TEL-002", detail="unknown warm-transfer callback action.")
...
def _validate_warm_transfer_id(value: str) -> str:
    if not _WARM_TRANSFER_ID.fullmatch(value):
        raise VoiceyError("VY-TEL-012", detail="invalid warm-transfer id.")
    return value


def _validate_conference_sid(value: str) -> str:
    if not _CONFERENCE_SID.fullmatch(value):
        raise VoiceyError("VY-TEL-008", detail="invalid Twilio ConferenceSid.")
    return value
...
def _status_event(
    ...
    raise VoiceyError("VY-TEL-008", detail=f"unknown Twilio CallStatus {status!r}.")


def _validate_call_sid(value: str) -> str:
    if not _CALL_SID.fullmatch(value):
        raise VoiceyError("VY-TEL-008", detail="invalid Twilio CallSid.")
    return value


def _validate_recording_sid(value: str) -> str:
    if not _RECORDING_SID.fullmatch(value):
        raise VoiceyError("VY-TEL-008", detail="invalid Twilio RecordingSid.")
    return value
