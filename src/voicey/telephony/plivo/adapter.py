...
class PlivoAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        if not _AUTH_ID.fullmatch(self.auth_id):
            raise VoiceyError("VY-TEL-002", detail="PLIVO_AUTH_ID is missing or invalid.")
        if not self._auth_token:
            raise VoiceyError("VY-TEL-002", detail="PLIVO_AUTH_TOKEN is required.")
        ...
        if (
            ...
            raise VoiceyError("VY-TEL-002", detail="Plivo base URL must be normalized HTTPS.")
    ...
    async def download_recording(
        ...
        *,
        ...
        if (
            ...
            raise VoiceyError("VY-TEL-009", detail="Plivo recording URL is not safe HTTPS.")
        if max_bytes <= 0:
    ...
    def parse_event(self, request: TelephonyRequest) -> CallEvent:
        ...
        if recording_url or recording_id:
            if not recording_url or not recording_id:
                raise VoiceyError("VY-TEL-008", detail="Plivo recording callback is incomplete.")
            return CallEvent(
    ...
    def _owned_number(self, number: str) -> dict[str, object]:
        ...
        if returned != normalized:
            raise VoiceyError("VY-TEL-003", detail="Plivo number ownership could not be proved.")
        return response
    ...
    def _ensure_application(self, target: PipecatTarget) -> tuple[str, bool]:
        ...
        if len(matches) > 1:
            raise VoiceyError("VY-TEL-006", detail="duplicate managed Plivo applications.")
        if matches:
            app = matches[0]
            if (
...
def _objects(document: Mapping[str, object], *, operation: str) -> list[dict[str, object]]:
    ...
    if not isinstance(raw, list):
        raise VoiceyError("VY-TEL-011", detail=f"Plivo {operation} is malformed.")
...
def _direction(value: str) -> Literal["inbound", "outbound"] | None:
    ...
    if normalized in {"outgoing", "outbound"}:
        return "outbound"
    raise VoiceyError("VY-TEL-008", detail="Plivo callback direction is invalid.")
...
def _form_value(form: object | None, name: str, *, required: bool = True) -> str:
    ...
    if value is None or value == "":
        if required:
            raise VoiceyError("VY-TEL-008", detail=f"Plivo callback lacks {name}.")
        return ""
    if isinstance(value, (str, int, float, bool)):
        return str(value)
    raise VoiceyError("VY-TEL-008", detail=f"Plivo callback {name} is malformed.")


def _provider_id(value: str, *, field_name: str) -> str:
    if not _PROVIDER_ID.fullmatch(value):
        raise VoiceyError("VY-TEL-008", detail=f"invalid Plivo {field_name}.")
    return value
...
def _optional(value: object) -> str | None:
    ...
    if isinstance(value, (str, int, float)):
        return str(value)
    raise VoiceyError("VY-TEL-011", detail="Plivo response field has an invalid scalar type.")
