...
from voicey.obs.records import NewCall, SQLiteCallRecordStore
...
class SQLiteRepository(SQLiteCallRecordStore):
        ...
        *,
        ...
        async with self._write_lock:
            try:
                ...
                raise VoiceyError("VY-RES-008", detail=str(exc)) from exc
        return CallLease(
    ...
    async def renew_lease(
        ...
        *,
        ...
        async with self._write_lock:
            ...
            await database.commit()
        if changed != 1:
            raise VoiceyError("VY-RES-006", detail=lease.call_id)
    ...
    async def _terminal_event_in_transaction(self, call_id: str) -> PersistedEvent:
        ...
        if row is None:
            raise VoiceyError("VY-RES-007", detail=call_id)
        return _event_from_row(row)
...
def _utc(value: datetime | None) -> datetime:
    ...
    if current.tzinfo is None or current.utcoffset() is None:
        raise VoiceyError("VY-RES-008", detail="timestamp must be timezone-aware.")
