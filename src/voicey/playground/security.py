...
_SESSION_KEY_CONTEXT = b"voicey/web-session/v1"
    ...
    def __init__(
        ...
        *,
        ...
        if limit < 1 or window_s <= 0:
            raise VoiceyError("VY-WEB-003", detail="rate limit and window must be positive.")
        self.limit = limit
...
class SessionTokenManager:
        ...
        *,
        ...
        async with self._lock:
            ...
            if state is None or state.identity != identity:
                raise VoiceyError("VY-WEB-001", detail="session was not issued here.")
            if state.pc_id is None:
                if pc_id is not None or state.reserved:
                    raise VoiceyError("VY-WEB-001", detail="session token was replayed.")
                state.reserved = True
            elif pc_id is None or not secrets.compare_digest(state.pc_id, pc_id):
                raise VoiceyError("VY-WEB-001", detail="session is bound to another peer.")
            return identity
        ...
        *,
        ...
        async with self._lock:
            ...
            if (
                ...
                raise VoiceyError("VY-WEB-001", detail="session cannot be bound.")
            state.pc_id = pc_id
    ...
    async def snapshot(self, session_id: str) -> SessionSnapshot:
        async with self._lock:
            ...
            if state is None:
                raise VoiceyError("VY-WEB-001", detail="browser session does not exist.")
    ...
    def _decode(self, token: str) -> WebSessionIdentity:
        try:
            ...
            raise VoiceyError("VY-WEB-001", detail="invalid browser session token.") from exc
...
class OriginPolicy:
    ...
    def validate(self, request: Request) -> None:
        ...
        if origin not in self.allowed_origins:
            raise VoiceyError("VY-WEB-002", detail=f"origin {origin!r} is not allowed.")
...
class WebSessionSecurity:
    ...
    def update_agent(self, agent_name: str, allowed_origins: frozenset[str]) -> None:
        ...
        if not secrets.compare_digest(agent_name, self.tokens.agent_name):
            raise VoiceyError(
                "VY-WEB-005",
                detail="changing agent.name requires restarting voicey dev.",
            )
...
def _bearer(value: str | None) -> str:
    if value is None or not value.startswith("Bearer ") or len(value) <= 7:
        raise VoiceyError("VY-WEB-001", detail="Authorization bearer token is required.")
    return value[7:]


def _identity(value: object) -> WebSessionIdentity:
    if not isinstance(value, WebSessionIdentity):
        raise VoiceyError("VY-WEB-001", detail="runtime received an invalid session identity.")
    return value
...
def _origin(value: str) -> str:
    ...
    if (
        ...
        raise VoiceyError("VY-WEB-002", detail=f"{value!r} is not an HTTP(S) origin.")
