...
from voicey.obs.records import CallRecord, CallStatus
from voicey.playground.security import SessionTokenManager
...
class PlaygroundSettings:
    ...
    def __post_init__(self) -> None:
        if _origin(self.admin_origin) != self.admin_origin:
            raise VoiceyError("VY-WEB-002", detail="admin_origin must be a normalized origin.")
        if _origin(self.public_origin) != self.public_origin:
            raise VoiceyError("VY-WEB-002", detail="public_origin must be a normalized origin.")
...
class PlaygroundService:
    ...
    def _install_public_browser_boundary(self) -> None:
        @self.public_app.exception_handler(VoiceyError)
        async def public_voicey_error_handler(  # pyright: ignore[reportUnusedFunction]
            _request: Request,
            error: VoiceyError,
        ) -> JSONResponse:
            return _voicey_error_response(error)
    ...
    def _build_admin_app(self) -> FastAPI:
        app = FastAPI(
            title=f"voicey playground:{self.agent.name}",
            docs_url=None,
            redoc_url=None,
            openapi_url=None,
        )
        ...
        async def recording(  # pyright: ignore[reportUnusedFunction]
            ...
            if value is None:
                raise VoiceyError("VY-OBS-003", detail=f"recording for {call_id}")
    ...
    async def _authorize_admin(self, request: Request) -> None:
        ...
        if not secrets.compare_digest(request.headers.get("host", ""), expected_host):
            raise VoiceyError("VY-WEB-004", detail="admin host does not match its listener.")
        origin = request.headers.get("origin")
        if origin is not None and not secrets.compare_digest(origin, self.settings.admin_origin):
            raise VoiceyError("VY-WEB-002", detail="admin browser origin is not allowed.")
        if self.admin_authorizer is not None and not await self.admin_authorizer(request):
            raise VoiceyError("VY-WEB-004", detail="integrator authorization failed.")
...
def _event_payload(event: PersistedEvent) -> dict[str, object]:
    try:
        ...
        raise VoiceyError("VY-RES-007", detail=event.event_id) from exc
    if not isinstance(value, dict):
        raise VoiceyError("VY-RES-007", detail=event.event_id)
    return dict(cast("dict[str, object]", value))


def _voicey_error_response(error: VoiceyError) -> JSONResponse:
    status = {
        "VY-WEB-001": 404,
        "VY-WEB-002": 403,
        "VY-WEB-003": 429,
...
def _origin(value: str) -> str:
    ...
    if (
        ...
        raise VoiceyError("VY-WEB-002", detail=f"{value!r} is not an origin.")
...
def _connect_origin(value: str) -> str:
    ...
    if (
        ...
        raise VoiceyError("VY-WEB-002", detail=f"{value!r} is not a connect origin.")
    return value.removesuffix("/")
