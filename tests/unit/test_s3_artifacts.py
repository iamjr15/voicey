...
class _MissingBodyS3(_MemoryS3):
    def get_object(self, **kwargs: object) -> Mapping[str, object]:
        self._check()
        return {"Metadata": {"voicey-sha256": "0" * 64}}
...
async def test_s3_artifacts_are_namespaced_checksummed_and_idempotently_deleted() -> None:
    ...
    stored = client.objects[("voicey-artifacts", "production/relay/recordings/call.wav")]
    assert stored == (b"audio", {"voicey-sha256": hashlib.sha256(b"audio").hexdigest()})
    ...
    assert client.deleted == [
        ("voicey-artifacts", "production/relay/recordings/call.wav"),
        ("voicey-artifacts", "production/relay/recordings/call.wav"),
    ]
...
async def test_s3_round_trip_removes_probe_and_detects_tampering() -> None:
    client = _MemoryS3()
    store = S3ArtifactStore("voicey-artifacts", client=cast("S3Client", client))
    ...
    await store.put("recordings/call.wav", b"audio")
    body, _ = client.objects[("voicey-artifacts", "recordings/call.wav")]
    client.objects[("voicey-artifacts", "recordings/call.wav")] = (
        body,
        {"voicey-sha256": "0" * 64},
    )
    with pytest.raises(VoiceyError) as caught:
        await store.read("recordings/call.wav")
    assert caught.value.code == "VY-ART-002"
    assert client.last_body is not None
    assert client.last_body.closed_by_store
...
async def test_s3_transport_failures_are_catalogued() -> None:
    ...
    with pytest.raises(VoiceyError) as readiness:
        await store.ready()
    assert readiness.value.code == "VY-ART-003"
...
@pytest.mark.parametrize(
    ...
    [
        ("Bad_Bucket", None, ""),
        ("192.168.1.1", None, ""),
        ("voicey-artifacts", "http://objects.example.test", ""),
        (
            "voicey-artifacts",
            "https://user:secret@objects.example.test",  # pragma: allowlist secret
            "",
        ),  # pragma: allowlist secret
        ("voicey-artifacts", "https://objects.example.test/path", ""),
        ("voicey-artifacts", None, "../escape"),
    ],
)
def test_s3_configuration_rejects_unsafe_values(
    ...
    assert caught.value.code in {"VY-ART-001", "VY-ART-003"}
...
def test_s3_configuration_accepts_loopback_http_and_rejects_partial_credentials() -> None:
    ...
    assert caught.value.code == "VY-ART-003"
...
def test_s3_builds_the_installed_client_with_explicit_security_settings(
    ...
    assert store.bucket == "voicey-artifacts"
...
async def test_s3_rejects_missing_body_and_round_trip_mismatch(
    ...
    assert missing.value.code == "VY-ART-002"
    ...
    assert mismatch.value.code == "VY-ART-003"
    assert not client.objects
