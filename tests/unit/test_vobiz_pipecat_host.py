"""Vobiz-specific Pipecat host routing and media lifecycle tests."""
...
from voicey.runtimes.pipecat.host import (
    ...
    PipecatHostSettings,
    VobizRuntimeAdapter,
    _vobiz_handshake,  # pyright: ignore[reportPrivateUsage]
    vobiz_transport_params,
)
...
def identify() -> str:
    ...
    return "vobiz-host-test"


def _agent(*, record: bool = False) -> Agent:
    return Agent(
        name="vobiz-host-test",
        runtime="pipecat",
        models=Models(
            stt="deepgram/nova-3",
        ...
        tools=[identify],
        phone=Phone(provider="vobiz", number="+918071234567", record=record),
...
class _Session:
    ...
    async def wait(self) -> object:
        ...
        return await self.lifecycle.finish(cast("Any", self.reason))
...
class _WebSocket:
    url = URL("wss://voice.example/vobiz/media/token")
    headers: ClassVar[dict[str, str]] = {}
    client = SimpleNamespace(host="13.233.44.61")
    ...
    async def receive_text(self) -> str:
        ...
        if self.receive_count > 1:
            raise AssertionError("host consumed the first media frame")
...
async def _host(
    tmp_path: Path,
    *,
    adapter: _Vobiz | None = None,
    builder: _SessionBuilder | None = None,
    record: bool = False,
) -> tuple[PipecatHost, SQLiteRepository, _Vobiz, _SessionBuilder]:
    repository = SQLiteRepository(tmp_path / "vobiz-host.sqlite3")
    await repository.open()
    vobiz = adapter or _Vobiz()
    sessions = builder or _SessionBuilder()
    host = PipecatHost(
        ...
        vobiz=cast("VobizRuntimeAdapter", vobiz),
        session_builder=cast("PipecatSessionBuilder", sessions),
    )
    return host, repository, vobiz, sessions


def test_vobiz_transport_is_exactly_8khz_and_never_calls_plivo_api() -> None:
...
def test_vobiz_handshake_requires_start_pcmu_8khz() -> None:
    frame = {
        "event": "start",
        "start": {
            "callId": "vobiz-call",
    ...
    assert _vobiz_handshake(json.dumps(frame)) == ("vobiz-call", "vobiz-stream")


async def test_vobiz_answer_media_and_signed_terminal_callback_are_one_lifecycle(
    tmp_path: Path,
) -> None:
    ...
    assert target.ws_path.startswith("/vobiz/media/")
    assert adapter.recordings[0][0] == "vobiz-call"
    ...
    route = next(
        route for route in host.app.routes if getattr(route, "path", None) == "/vobiz/media/{token}"
    )
    ...
    active = await repository.get_call("vobiz-call")
    ...
    async with httpx.AsyncClient(
        ...
        terminal = await client.post(
            "/vobiz/events",
            data={"CallUUID": "vobiz-call", "CallStatus": "completed"},
        )
    await media_task
    persisted = await repository.get_call("vobiz-call")
...
async def test_vobiz_routes_fail_closed_before_reservation(tmp_path: Path) -> None:
    host, repository, _adapter, _sessions = await _host(
        tmp_path,
        adapter=_Vobiz(verified=False),
    )
    async with httpx.AsyncClient(
        transport=httpx.ASGITransport(app=host.app),
        base_url="https://voice.example",
        ...
        denied = await client.post(
            "/vobiz/answer",
            data={"CallUUID": "vobiz-denied", "CallStatus": "initiated"},
        )
