...
from voicey.cli.drafting import PromptDrafter
from voicey.cli.keys import KeyCheck
from voicey.cli.prompts import PromptChoice
...
class ScriptedPrompt:
    ...
    def select(self, message: str, choices: tuple[PromptChoice, ...]) -> str:
        ...
        if not self._selections:
            raise VoiceyError("VY-CLI-001", detail="missing selection flag")
    ...
    def multiselect(
        ...
        *,
        ...
        if not self._multiselections:
            raise VoiceyError("VY-CLI-001", detail="missing channels flag")
        return self._multiselections.pop(0)

    def text(self, message: str) -> str:
        self.text_calls.append(message)
        if not self._texts:
            raise VoiceyError("VY-CLI-001", detail="missing text flag")
        return self._texts.pop(0)

    def secret(self, message: str) -> str:
        del message
        if not self._secrets:
            raise VoiceyError("VY-CLI-004", detail="missing key")
...
async def test_flag_only_web_wizard_produces_working_scaffold(tmp_path: Path) -> None:
    ...
    assert result.next_step.endswith("voicey dev")
    ...
    assert "VOICEY_WEBHOOK_SECRET=" in env_payload
    assert "dg-key" not in env_payload
    assert "ant-key" not in env_payload
    assert "car-key" not in env_payload
    assert ManifestStore(result.project_dir / "voicey.jsonc").load() == result.manifest
...
async def test_flag_only_appointment_recipe_copies_authored_native_source(
    ...
    assert "VOICEY_TRANSFER_NUMBER=" in (result.project_dir / ".env.example").read_text(
        encoding="utf-8"
    )
...
async def test_resume_uses_checkpoint_without_reasking_saved_answers(tmp_path: Path) -> None:
    ...
    assert missing.value.code == "VY-CLI-001"
    checkpoint_payload = (project_dir / "voicey.jsonc").read_text(encoding="utf-8")
...
async def test_unknown_runtime_fails_before_key_or_scaffold_writes(tmp_path: Path) -> None:
    ...
    assert caught.value.code == "VY-CLI-005"
...
async def test_livekit_web_wizard_produces_native_quickstart(tmp_path: Path) -> None:
    ...
    assert "voicey[livekit]" in (result.project_dir / "pyproject.toml").read_text(encoding="utf-8")
...
async def test_livekit_wizard_key_failures_are_actionable(tmp_path: Path) -> None:
    ...
    with pytest.raises(VoiceyError, match="keys add livekit"):
    ...
    with pytest.raises(VoiceyError, match="process environment"):
    ...
    with pytest.raises(VoiceyError, match="cannot be blank"):
    ...
    with pytest.raises(VoiceyError, match="failed three validation attempts"):
...
async def test_noninteractive_missing_keys_names_add_command(tmp_path: Path) -> None:
    ...
    assert "voicey keys add deepgram" in caught.value.detail
...
async def test_phone_wizard_selects_carrier_number_and_persists_resume_state(
    ...
    assert "voicey[pipecat,twilio]" in (project_dir / "pyproject.toml").read_text(encoding="utf-8")
    with pytest.raises(VoiceyError) as duplicate:
        await wizard.run(project_dir, InitOptions(resume=True))
    assert duplicate.value.code == "VY-CLI-007"
...
async def test_wizard_rejects_invalid_phone_model_axis_and_missing_extra(
    ...
    with pytest.raises(VoiceyError) as phone:
        await wizard.run(
    ...
    assert phone.value.code == "VY-CLI-001"

    with pytest.raises(VoiceyError) as axis:
        await wizard.run(
            tmp_path / "axis-agent",
            InitOptions(
    ...
    assert axis.value.code == "VY-CLI-001"

    def no_extra(_module: str) -> None:
        return None

    monkeypatch.setattr("voicey.cli.wizard.importlib.util.find_spec", no_extra)
    with pytest.raises(VoiceyError) as extra:
        await wizard.run(
    ...
    assert extra.value.code == "VY-CLI-005"
...
async def test_resume_requires_flag_and_matching_name(tmp_path: Path) -> None:
    ...
    with pytest.raises(VoiceyError) as no_resume:
    ...
    assert no_resume.value.code == "VY-CLI-002"
    assert mismatch.value.code == "VY-CLI-002"
...
async def test_invalid_process_key_and_blank_paste_fail_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(VoiceyError) as process:
