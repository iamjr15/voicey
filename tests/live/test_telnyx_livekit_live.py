...
def _config() -> TelnyxLiveKitSipConfig:
    return TelnyxLiveKitSipConfig(
        number=_required("VOICEY_TELNYX_LIVE_FROM"),
        agent_name=_required("VOICEY_LIVEKIT_AGENT_NAME"),
        livekit_sip_uri=_required("VOICEY_LIVEKIT_SIP_URI"),
        auth_username=_required("VOICEY_TELNYX_SIP_USERNAME"),
        auth_password=_required("VOICEY_TELNYX_SIP_PASSWORD"),
        anchorsite=os.environ.get("VOICEY_TELNYX_ANCHORSITE", "Latency"),
    )
...
async def test_live_telnyx_livekit_paid_outbound_and_sip_status_mapping(
    ...
    try:
        result = await dialer.dial(
            from_number=_required("VOICEY_TELNYX_LIVE_FROM"),
            to_number=_required("VOICEY_TELNYX_LIVE_TO"),
            room_name=_required("VOICEY_LIVEKIT_CERT_ROOM"),
            participant_identity="voicey-telnyx-cert-callee",
