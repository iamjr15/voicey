...
def _adapter(tmp_path: Path, name: str) -> tuple[PlivoAdapter, TelephonyLedger]:
    ...
    return (
        PlivoAdapter(
            ...
            ledger=ledger,
            expected_public_base=os.environ.get("VOICEKIT_LIVE_PUBLIC_BASE"),
        ),
        ledger,
    )
...
def test_plivo_live_account_and_owned_number_are_ready(tmp_path: Path) -> None:
    ...
    try:
        ...
        matches = [
            ...
            if number.number == _required("VOICEKIT_PLIVO_LIVE_FROM")
        ]
    finally:
        ledger.close()
...
def test_plivo_live_route_point_and_crash_safe_restore(tmp_path: Path) -> None:
    ...
    try:
        token = adapter.point_inbound(
            _required("VOICEKIT_PLIVO_LIVE_FROM"),
            PipecatTarget(_required("VOICEKIT_LIVE_PUBLIC_BASE")),
        )
...
def test_plivo_live_paid_pstn_amd_dtmf_recording_and_transfer(tmp_path: Path) -> None:
    ...
    call_id = adapter.start_call(
        _required("VOICEKIT_PLIVO_LIVE_FROM"),
        _required("VOICEKIT_PLIVO_LIVE_TO"),
        PipecatTarget(_required("VOICEKIT_LIVE_PUBLIC_BASE")),
    ...
    try:
        time.sleep(float(os.environ.get("VOICEKIT_LIVE_ANSWER_WAIT_SECONDS", "15")))
        adapter.send_dtmf(call_id, "12#")
        time.sleep(2)
        adapter.cold_transfer(call_id, _required("VOICEKIT_PLIVO_TRANSFER_TO"))
        time.sleep(5)
    finally:
        try:
...
async def test_plivo_live_recording_ingests_to_engine_storage(tmp_path: Path) -> None:
    ...
    try:
        key = await adapter.download_recording(
            _required("VOICEKIT_PLIVO_LIVE_RECORDING_URL"),
