...
carrier that is not represented by a first-party adapter. Voicey manages
...
- `VOICEY_SIP_ADDRESS` as `host` or `host:port`, without `sip:` or
  credentials;
- `VOICEY_SIP_USERNAME`;
- `VOICEY_SIP_PASSWORD`;
- `VOICEY_SIP_TRANSPORT` as `udp`, `tcp`, or `tls`;
- `VOICEY_SIP_MEDIA_ENCRYPTION` as `disable`, `allow`, or `require`;
- optional comma-separated `VOICEY_SIP_ALLOWED_ADDRESSES` CIDRs.
...
Voicey creates or verifies only these LiveKit resources:
...
configure the reverse route to `VOICEY_SIP_ADDRESS`, make the authentication
values agree, and apply matching network and media policies. `voicey dev`
cannot mutate or restore that external system.

Provisioning metadata records `managed_by=voicey`, provider `sip`, Beta
...
or a changed secret fingerprint returns `VY-TEL-006`.
...
ambiguous; voicey does not retry a potentially successful mutation.
