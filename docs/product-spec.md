...
- Voicey-created container virtual environments are bootstrapped without
  `pip`; pinned `uv` installs dependencies in the build stage. The runtime
  therefore carries application dependencies, not a second package installer
  or its vendored build-tool dependency set. Each final stage also removes the
  base image's system pip; Pipecat Cloud otherwise retains the system packages
  supplied by its separately pinned upstream base image.
...
- **Runtime compatibility:** each release pins tested ranges of `pipecat-ai`/`livekit-agents`; the initial empirically certified windows are Pipecat `>=1.6.0,<1.6.1` and LiveKit Agents `>=1.6.7,<1.6.8`, while generated project extras remain exact at the tested versions. The Pipecat extra constrains its NLTK dependency to `>=3.10.2,<4`, retaining the installed Pipecat contract while excluding versions affected by PYSEC-2026-3726. The LiveKit extra also pins `anthropic==0.120.0`: the certified LiveKit Anthropic plugin passes an `httpx.AsyncClient`, while Anthropic 1.0 requires `httpx2` and fails before media connection. A compatibility table lives in docs; CI installs every declared edge on Python 3.11 and 3.14 and exercises all first-party recipes. Out-of-range installs warn loudly (not fail) with the compatibility-table link; a missing runtime remains a setup failure.
...
- Release cadence target: minor every 4–6 weeks; canary channel (`pip install voicey --pre`) exercised from an installed wheel by every first-party recipe on both native runtimes before stable. Stable artifact preparation requires a green canary report for the same release line. The Python release workflow accepts exact `v<version>` tags only, stores canaries as private CI artifacts, and publishes stable artifacts only from an isolated job after the protected `pypi` environment is approved. The independently versioned npm metadata package uses exact `npm-v<version>` tags, so one registry's release can never trigger the other's workflow. Publishing jobs receive only GitHub's short-lived OIDC identity and use registry Trusted Publishing; build/test code never receives `id-token: write`, and no registry credential is stored in GitHub. Configuring required reviewers and binding the repository's exact owner/name/workflow/environment identity on PyPI remain explicitly authorized setup. An approved bootstrap upload before that binding receives a project-scoped token only through the publisher process environment, never a command argument, repository file, generated artifact, or literal shell-history entry. If an unclaimed project requires an account-wide token for its first upload, that token is revoked immediately after project creation and replaced by a project-scoped token.
