...
| `pipecat-cloud` | secret-free context + non-root image + deploy config; operator builds/pushes an immutable tag required by the pinned CLI | secret set, synced from an owner-only temporary file | automatic for Twilio/Vobiz/Plivo via the companion's stable hosted-answer URL; Telnyx requires its TeXML app to use that URL and an explicit acknowledgement | platform session + durable relay; paid phone call and delivered result for phone projects |
