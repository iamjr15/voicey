...
|---|---|---|
| `VY-CFG-*` | `Agent` or `voicey.jsonc` | Run `voicey doctor`, then correct the named field |
| `VY-CLI-*` | command input, state, confirmation, or provider key | Run the printed command/fix exactly |
| `VY-TEL-*` | carrier control, signature, number, SIP, transfer | Open the selected [carrier guide](index.md#carriers) |
| `VY-RUN-*` | runtime startup, admission, media, drain | Confirm the [compatibility table](compatibility.md) and runtime guide |
| `VY-RES-*` | lifecycle, result signing/delivery, recording | Inspect [Results and webhooks](results-webhooks.md) |
| `VY-OBS-*` | protected records, metrics, OTLP | Inspect disk/permissions and [Observability](observability.md) |
| `VY-TST-*` | scenario discovery, simulator, native evaluator | Read [Testing](testing.md) |
| `VY-DEP-*` | deployment plan, identity, smoke, rollback | Open the exact target guide under `docs/deploy/` |
| `VY-REL-*` | results relay or companion | Read [Results relay](results-relay.md) |
| `VY-UPG-*` | uv upgrade or recipe drift | Read [Upgrading](upgrading.md) |
| `VY-SEC-*` | permissions, symlink, or secret boundary | Stop mutation and restore an owner-only regular file |
...
Confirm you opened the admin URL printed by `voicey dev`, allowed microphone
...
while a voicey rollback token is active.
...
`voicey calls redeliver <call-id-or-event-id> --yes`. Redelivery preserves the
event id and immutable body.

### An upgrade reports recipe drift

Run `voicey recipes update-check --json`. Treat the output as a three-way
...
Voicey warns but continues. Consult [Compatibility](compatibility.md), pin
