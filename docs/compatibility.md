...
| Google Gemini | `google/gemini-3.5-flash` | Live free-tier tool-calling succeeded on 2026-08-23; 2.5 returned 404 for this account, while 3.6 repeatedly returned 504 for the deployed agent's full prompt/tool schema |
...
- LiveKit cloud smoke: issue `AccessToken(...).with_grants(VideoGrants(...))`,
  ...
  the non-publishing participant before deleting the room. The installed room
  service can return `ServerError(code="not_found")` because LiveKit deletes an
  empty room first; smoke cleanup treats only that response as idempotent.
