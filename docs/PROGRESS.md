...
- **Current unit:** P1.8 `cli/`
- **Next task:** build the complete capability-gated, resumable guided wizard, in-flow key collection/validation, recipe registry, dev/phone/call/numbers/keys/calls surfaces, doctor, error catalog presentation, next-step engine, flag twins, and JSON paths
- **Completed:** all P0 units; P1.1 configuration; P1.2 observability; P1.3 repository/results reliability; P1.4 tools; P1.5 Twilio; P1.6 Pipecat runtime; P1.7 deterministic ngrok/cloudflared/manual tunnels with supervised teardown and authenticated WebSocket probe
