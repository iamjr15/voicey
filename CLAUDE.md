...
- Phase: **P4, unit 4 (upgrade and drift tooling)**. P0–P3 and P4.1–P4.3 local automation are implemented. P4.3 adds the Railway CLI target with explicit paid-resource planning, exact adoption, managed Postgres/private bucket references, stdin-only credentials, pre-deploy migration/object/fencing proof, two-replica release, signed readiness, credential rotation, resumable checkpoints, and reverse created-only rollback. Railway CLI 5.30.1 and disposable PostgreSQL 17 ran green locally; the full-suite checkpoint is 1,036 passed, 41 truthful skips, and 90.15% branch coverage. The authenticated Railway deploy, exact 24-hour soak, live rolling deploys, and other credentialed gates remain pending-time/pending-live in `docs/GAPS.md`. Continue to verify every Pipecat/LiveKit symbol against the installed pins. See `docs/PROGRESS.md`.
