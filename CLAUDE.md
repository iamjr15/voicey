...
- Phase: **P4, unit 3 (Railway deployment target)**. P0–P3 and P4.1–P4.2 local automation are implemented. P4.2 adds bounded runtime-neutral Prometheus metrics, a separately bound exporter, optional PII-safe OTLP/HTTP call tracing, fork-safe runtime integration, durable delivery/DLQ observation, both-runtime config mapping, and an actual loopback Prometheus+OTLP wire gate. The P4.2 full-suite checkpoint is 1,019 passed, 40 honest live skips, and 90.11% branch coverage. The exact 24-hour soak, live rolling deploys, and other credentialed gates remain pending-time/pending-live in `docs/GAPS.md`. Continue to verify every Pipecat/LiveKit symbol against the installed pins. See `docs/PROGRESS.md`.
