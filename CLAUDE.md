...
- Phase: **P3, unit 4 (Tier-3 live PSTN loopback)**. P0, P1, P2, and P3.1–P3.3 are implemented locally. Plivo now ships signed/fenced Pipecat Voice API media plus the officially documented LiveKit inbound-TCP/secure-outbound Zentrunk path; generic SIP is an explicit LiveKit-only, operator-managed Beta path. Their credentialed, external-route, paid PSTN, and physical gates remain honestly pending with exact commands in `docs/GAPS.md`. The current full suite is 807 passed, 28 honest live skips, and 90.03% branch coverage; strict ruff/format/pyright are green. Continue to verify every Pipecat/LiveKit symbol against the installed pins. See `docs/PROGRESS.md`.
