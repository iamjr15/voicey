...
- Phase: **P4, unit 5 (release engineering and compatibility)**. P0–P3 and P4.1–P4.4 local automation are implemented. P4.4 adds a committed exact recipe baseline, read-only five-state three-way drift with source-free JSON and AI merge guidance, and a confirmed lockfile-only uv upgrade transaction with stable/canary validation, fresh-process inspection, project-source byte checks, and rollback. The real disposable-project uv 0.11.7 gate is green; the full-suite checkpoint is 1,081 passed, 41 truthful skips, and 90.25% branch coverage. The authenticated Railway deploy, exact 24-hour soak, live rolling deploys, and other credentialed gates remain pending-time/pending-live in `docs/GAPS.md`. Continue to verify every Pipecat/LiveKit symbol against the installed pins. See `docs/PROGRESS.md`.
