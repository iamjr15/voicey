...
- Phase: **P4, unit 1 (chaos/soak/drain hardening)**. P0–P3 are implemented locally. P3.6 closed the final parity exclusion with a consent-gated Pipecat/Twilio conference bridge, private human briefing, signed acceptance/status/conference callbacks, durable mutation fencing, privacy-minimized state, and exact failure/restart behavior. The full suite is 979 passed, 40 honest live skips, and 90.04% branch coverage; strict ruff/format/pyright are green. The release-shaped P3 aggregate, including a disposable PostgreSQL 17 run, has all eight local rows green; six provider/cloud/handset rows remain pending-live with exact commands in `docs/GAPS.md`. Continue to verify every Pipecat/LiveKit symbol against the installed pins. See `docs/PROGRESS.md`.
