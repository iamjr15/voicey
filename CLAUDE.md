...
- Phase: **P4, unit 6 (documentation, security, rename, and aggregate completion)**. P0–P3 and P4.1–P4.5 local automation are implemented. P4.5 adds executable SemVer/deprecation policy, non-fatal certified-runtime warnings, exact Python 3.11/3.14 edge evidence, canonical config/webhook/CLI snapshots with docs enforcement, and a private-artifact release workflow whose stable path requires same-line green canary evidence. The real installed-wheel canary exercised all four recipes on both pinned runtimes; the full suite is 1,118 passed, 41 truthful skips, and 90.38% branch coverage. The authenticated Railway deploy, exact 24-hour soak, live rolling deploys, and other credentialed gates remain pending-time/pending-live in `docs/GAPS.md`. Continue to verify every Pipecat/LiveKit symbol against the installed pins. See `docs/PROGRESS.md`.
