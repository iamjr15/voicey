...
- Phase: **P3, unit 5 (cloud deploys + relay + Fly companion)**. P0, P1, P2, and P3.1–P3.4 are implemented locally. The paid live tier now has native Pipecat/Twilio and LiveKit/SIP caller paths, exact acknowledgment/budget preflight, black-box evidence, and a protected nightly workflow; its real paid calls remain pending because the required credentials and destination are unavailable. The current full suite is 838 passed, 28 honest live skips, and 90.15% branch coverage; strict ruff/format/pyright are green. Continue to verify every Pipecat/LiveKit symbol against the installed pins. See `docs/PROGRESS.md`.
