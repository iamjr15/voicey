...
eval_agent = agent.model_copy(
    update={
        "phone": agent.phone
        or Phone(provider="twilio", number="+15555550198", inbound=True, outbound=False),
...
async def bot(runner_args: RunnerArguments) -> None:
    ...
    if not isinstance(runner_args, EvalRunnerArguments):
        raise VoiceyError("VY-RUN-001", detail="eval_bot.py must run with `-t eval`.")
